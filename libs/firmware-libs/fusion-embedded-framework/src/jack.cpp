...
JackClient::JackClient(const std::string &name, AudioTask *task)
    : name(name), task(task), client_active(false)
{
    jack_status_t jack_status;
    client = jack_client_open(name.c_str(), JackNullOption, &jack_status, NULL);
...
int_fast32_t JackClient::get_sample_rate()
{
    return jack_get_sample_rate(client);
}


int_fast32_t JackClient::get_frame_size()
{
    return jack_get_buffer_size(client);
}


jack_client_t *JackClient::get_jack_client()
{
    return client;
}


void JackClient::attach_block(Jack *jack_block)
{
    jack_blocks.insert(jack_block);
}


const std::string &JackClient::get_name() const
{
    return name;
}
...
void *JackClient::jack_thread_helper(void *arg)
{

...
void validateSplRange(List<double> splRange, {String? context}) {
  if (splRange.length != 2) {
    throw ArgumentError('SPL range must contain exactly 2 values [min, max]${context != null ? ' ($context)' : ''}');
  }
...
void validateNotEmpty<T>(List<T> list, String listType, {String? context}) {
  if (list.isEmpty) {
    throw ArgumentError('$listType cannot be empty${context != null ? ' ($context)' : ''}');
  }
}
...
void validatePositive(double value, String valueType, {String? context}) {
  if (value <= 0) {
    throw ArgumentError('$valueType must be positive${context != null ? ' ($context)' : ''}');
  }
}
...
void validateNonNegative(double value, String valueType, {String? context}) {
  if (value < 0) {
    throw ArgumentError('$valueType cannot be negative${context != null ? ' ($context)' : ''}');
  }
}

/// Common validation fusion_acoustic_calculation_engine

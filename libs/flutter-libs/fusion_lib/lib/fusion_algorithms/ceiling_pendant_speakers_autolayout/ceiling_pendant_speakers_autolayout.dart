...
class Room {
  ...
  void _validateGeometry() {
    if (roomType == RoomType.asymmetrical) {
      ...
      if (area > 10000) { // Large room warning
...
class AutoSpeakerPlacement {
  ...
  static double _optimizeGridSpacingForAsymmetricalRoom(
    Room room, 
    ...
    if (roomArea <= 20) {          // Small rooms (≤20 m²)
      maxPracticalSpeakers = 4;
    } else if (roomArea <= 50) {   // Medium rooms (20-50 m²)
      maxPracticalSpeakers = 8;
    } else if (roomArea <= 100) {  // Large rooms (50-100 m²)
      maxPracticalSpeakers = 12;
    } else {                       // Very large rooms (>100 m²)
  ...
  static List<Point2D> _applyFinalSpeakerCountLimit(
    List<Point2D> speakers, 
    Room room, 
    List<String> steps
  ) {
    ...
    if (roomArea <= 25) {          // Small rooms (≤25 m²)
      maxAllowedSpeakers = 6;
    } else if (roomArea <= 60) {   // Medium rooms (25-60 m²)
      maxAllowedSpeakers = 10;
    } else if (roomArea <= 120) {  // Large rooms (60-120 m²)
      maxAllowedSpeakers = 16;
    } else {                       // Very large rooms (>120 m²)
    ...
    List<Point2D> optimizedSpeakers = _selectOptimalSpeakers(
      speakers, 
      maxAllowedSpeakers, 
      room
    );
  ...
  static List<Point2D> _selectOptimalSpeakers(
    List<Point2D> allSpeakers,
    int targetCount,
    Room room
  ) {
      ...
      for (Point2D candidate in remaining) {
        ...
        for (Point2D selectedSpeaker in selected) {
          double distance = sqrt(
            pow(candidate.x - selectedSpeaker.x, 2) + 
            pow(candidate.y - selectedSpeaker.y, 2)
          );
  ...
  static bool _isSpeakerValidInAsymmetricalRoom(Point2D speakerPosition, double coverageRadius, Room room, double boundaryOverlapThreshold) {
    ...
    double minDistanceFromEdge = coverageRadius * 0.5;

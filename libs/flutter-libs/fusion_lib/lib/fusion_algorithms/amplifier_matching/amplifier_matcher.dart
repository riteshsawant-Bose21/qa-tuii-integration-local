...
class AmplifierMatcher {
  ...
  static List<CircuitWithPower> _computeAndSortCircuits(List<Circuit> circuits, Map<String, SpeakerModel> speakerDatabase) {
      ...
      switch (circuit.mode.toLowerCase()) {
        ...
        case 'lo-z':
          ...
          if (totalImpedance < 4.0) {
            throw AmpMatchingException('Circuit ${circuit.circuitId} impedance too low: ${totalImpedance.toStringAsFixed(2)} Ω');
          }
          break;
  ...
  static List<AmpAssignment> _initialAmpAllocation(List<CircuitWithPower> computedCircuits) {
    ...
    for (final circuitWithPower in computedCircuits) {
      ...
      final optimizedAmpModel = _applySmartChannelStrategy(ampModel, channelsNeeded, assignments.length, assignments);

      // Create new amplifier assignment if needed
      if (currentAmp == null || currentAmp.ampModel.name != optimizedAmpModel.name || currentAmp.circuits.length >= optimizedAmpModel.channels) {
        currentAmp = AmpAssignment(ampModel: optimizedAmpModel, circuits: []);
        assignments.add(currentAmp);
      }

      // Add circuit to current amplifier
      final updatedCircuits = List<Circuit>.from(currentAmp.circuits)..add(circuitWithPower.circuit);

      currentAmp = AmpAssignment(ampModel: currentAmp.ampModel, circuits: updatedCircuits);
  ...
  static AmpModel _applySmartChannelStrategy(AmpModel baseModel, int channelsRemaining, int existingAmpsCount, List<AmpAssignment> existingAssignments) {
    ...
    if (totalChannelsNeeded <= 4) {
      ...
      // 8 + 4 pattern
      return existingAmpsCount == 0 ? _find8ChannelVariant(baseModel) : _find4ChannelVariant(baseModel);
    } else {
  ...
  static AmpModel _find4ChannelVariant(AmpModel baseModel) {
    final variant = AmpCatalog.models.where((amp) => amp.peakPerChannel == baseModel.peakPerChannel && amp.channels == 4);
  ...
  static AmpModel _find8ChannelVariant(AmpModel baseModel) {
    final variant = AmpCatalog.models.where((amp) => amp.peakPerChannel == baseModel.peakPerChannel && amp.channels == 8);
    return variant.isNotEmpty ? variant.first : baseModel;
  }

  /// Steps 6-12: Apply power sharing optimization algorithm
  static List<AmpAssignment> _applyPowerSharingOptimization(List<AmpAssignment> assignments, Map<String, SpeakerModel> speakerDatabase) {
    ...
    final optimizedAssignments = _optimizeChannelAllocation(assignments, powerSharingInfo, speakerDatabase);
  ...
  static PowerSharingInfo _calculatePowerSharing(AmpAssignment assignment, int ampIndex, Map<String, SpeakerModel> speakerDatabase) {
    ...
    return PowerSharingInfo(ampIndex: ampIndex, usedChannels: assignment.circuits.length, netPowerSharing: netPowerSharing, channelLoads: channelLoads);
  }
  ...
  static List<AmpAssignment> _optimizeChannelAllocation(
    ...
    for (int ampIdx = 0; ampIdx < assignments.length; ampIdx++) {
      ...
      for (int circuitIdx = 0; circuitIdx < assignment.circuits.length; circuitIdx++) {
        ...
        movableCircuits.add(_MovableCircuit(circuitIndex: circuitIdx, ampIndex: ampIdx, power: power, circuit: circuit));
      }
    }
    ...
    final optimizedAssignments = assignments.map((a) => AmpAssignment(ampModel: a.ampModel, circuits: List<Circuit>.from(a.circuits))).toList();

    final workingPowerInfo = powerInfo
        .map(
          (info) => PowerSharingInfo(
    ...
    for (int i = 0; i < movableCircuits.length; i++) {
      ...
      for (int targetAmpIdx = 0; targetAmpIdx < workingPowerInfo.length; targetAmpIdx++) {
        ...
        if (targetAmpPower < currentAmpPower &&
            ...
            optimizedAssignments[targetAmpIdx].circuits.length < assignments[targetAmpIdx].ampModel.channels) {
  ...
  static List<AmpAssignment> _validateAndReduceSKUs(List<AmpAssignment> assignments, Map<String, SpeakerModel> speakerDatabase) {
    ...
    for (final assignment in assignments) {
      ...
      if (!validTier) {
        ...
        if (reducedAmp != null) {
          ...
          throw AmpMatchingException('Cannot find suitable amplifier tier for power requirement ${maxCircuitPower.toStringAsFixed(2)}W');
        }
      }

      finalAssignments.add(AmpAssignment(ampModel: finalAmp, circuits: assignment.circuits));
    }
  ...
  static bool _validateTierRule(AmpModel amp, double requiredPower) {
    ...
    for (int i = 0; i < sortedCatalog.length; i++) {
      ...
      final double nextTierPower = (i < sortedCatalog.length - 1) ? sortedCatalog[i + 1].peakPerChannel : double.infinity;
  ...
  static AmpModel? _findLowerTierAmp(AmpModel currentAmp, double requiredPower) {
    ...
    final sameChannelAmps = AmpCatalog.models.where((amp) => amp.channels == currentAmp.channels).toList();
  ...
  static AmpMatchingResult _createMatchingResult(List<AmpAssignment> assignments, List<CircuitWithPower> computedCircuits) {
    final totalPowerRequirement = computedCircuits.fold<double>(0.0, (sum, circuit) => sum + circuit.requiredPpk);

    final totalSystemCapacity = assignments.fold<double>(0.0, (sum, assignment) => sum + assignment.totalCapacity);

    final totalChannelsUsed = assignments.fold<int>(0, (sum, assignment) => sum + assignment.usedChannels);

    final totalChannelsAvailable = assignments.fold<int>(0, (sum, assignment) => sum + assignment.totalChannels);
...
class _MovableCircuit {
  ...
  const _MovableCircuit({required this.circuitIndex, required this.ampIndex, required this.power, required this.circuit});
}

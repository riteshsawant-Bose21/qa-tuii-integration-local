...
enum BleConnectionStatus { initial, connected, disconnected }

enum BluetoothStatus { initial, on, off }
...
class BleConnectionManager {
  ...
  static Future<void> listDevices() async {
    ...
    final StreamSubscription<List<ScanResult>> subscription = FlutterBluePlus.onScanResults.listen((List<ScanResult> results) {
    ...
    if (!isBTOn) {
      ...
      await FlutterBluePlus.startScan(withServices: <Guid>[ServiceUUIDs.getUUID(Services.fusion)]);
  ...
  static Future<bool> listServices(BluetoothDevice connectedDevice) async {
    ...
    try {
      ...
      for (BluetoothService service in services) {
        FusionLogger.log(tag: LogTag.ble, message: "ServiceId: ${service.serviceUuid}  ");
  ...
  Future<void> initialize(String? tempRemoteId) async {
    ...
    FusionLogger.log(tag: LogTag.ble, message: "Ble connection: initialize() called Device id $deviceId");

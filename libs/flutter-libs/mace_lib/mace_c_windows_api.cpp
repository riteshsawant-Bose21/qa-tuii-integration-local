// mace_c_windows_api.cpp

#define MACE_C_API_BUILD    // so functions are dllexport when building the DLL
#include "mace_c_api.h"
...
MACE_API const char* mace_get_lib_version()
{
    auto eng = EngineFactory::GetEngine();
    auto ver = eng ? eng->GetLibVersion() : std::string();
    return strdup(ver.c_str());  // uses malloc under the hood
}
...
MACE_API void mace_debug_speakers()
{
...
MACE_API EngineHandle mace_create_engine(const char* speakerDefsPath)
{
    ...
    return static_cast<EngineHandle>(reinterpret_cast<uint64_t>(eng.get()));
}

/// Stop & destroy engine
MACE_API void mace_destroy_engine()
{
...
MACE_API SurfaceHandle mace_add_polygon(
    EngineHandle /*e*/,
    const double* xyz,
    int           count
) {
...
MACE_API ClusterHandle mace_add_speaker_cluster(
    EngineHandle /*e*/,
...
/// Add field points
MACE_API FieldPointsHandle mace_add_field_points(
    EngineHandle    /*e*/,
    const double*   xyz,
    int             count
) {
...
/// Create a Group
MACE_API uint64_t mace_create_group()
{
...
/// Computes SPL for all default ISO center frequencies across multiple bands
                                           ...
                                           FieldPointsHandle fph)
{
    ...
    if (thirdFreqs.empty() && nThird > 0) {
        // Optional: synthesize / leave empty
    }

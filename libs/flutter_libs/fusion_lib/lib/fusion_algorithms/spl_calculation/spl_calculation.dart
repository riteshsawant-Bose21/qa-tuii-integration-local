...
SplMultiMountResult calculateSpl(SplInput input) {
  ...
  developer.log('Target SPLs - Min: ${minTargetSpl.toStringAsFixed(2)}, '
  ...
  for (final rawType in input.mountingType) {
    ...
    try {
      ...
      final distance = calculateDistance(
        mountType,
        input.speakerHeight,
        input.listenerHeight,
      );

      developer.log('Calculated distance (feet): ${distance.toStringAsFixed(2)} '
      ...
      developer.log('SPL Required - Min: ${splReqMin.toStringAsFixed(2)}, '
...
SplMultiMountResult calculateSplForApplication({
  ...
  // This would require importing the application ranges from constants
  // For now, let the caller specify the target range directly
  throw UnimplementedError(
    'Use calculateSpl() with explicit targetSplRange. '
  );
}

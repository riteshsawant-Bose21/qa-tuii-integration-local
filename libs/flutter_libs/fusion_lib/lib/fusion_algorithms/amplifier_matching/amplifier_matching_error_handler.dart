...
class InvalidCircuitException extends AmpMatchingException {
  const InvalidCircuitException(String message, {String step = 'VALIDATION', Map<String, dynamic>? context})
...
class CatalogException extends AmpMatchingException {
  const CatalogException(String message, {String step = 'CATALOG_ACCESS', Map<String, dynamic>? context})
      : super(message, step: step, context: context);
}
...
class AmpMatchingErrorHandler {
  ...
  static ValidationResult validateSpeakerDatabase(Map<String, Speaker> speakers, List<Circuit> circuits) {
    final errors = <String>[];
    final warnings = <String>[];
    ...
    return ValidationResult(
      ...
      warnings: warnings,
      metadata: {
        'total_speaker_models': speakers.length,
        'used_speaker_models': usedModels.length,
        'missing_models': missingModels.length,
      },
    );
  }
  
  /// Validate individual speaker specification
  static List<String> _validateSpeakerSpec(String model, Speaker speaker) {
    final errors = <String>[];
  ...
  static ValidationResult validateAmplifierCatalog(List<AmpModel> amplifiers) {
    ...
    return ValidationResult(
      ...
      warnings: warnings,
      metadata: {
        'amplifier_count': amplifiers.length,
        'power_tiers': powerTiers.length,
        'min_power': sortedTiers.first,
        'max_power': sortedTiers.last,
      },
    );
  }
  
  /// Validate circuit power calculations
  static void validateCircuitPower(Circuit circuit, double calculatedPower, Speaker? speaker) {
    ...
    if (circuit.mode.toLowerCase().contains('hi') && speaker != null) {
      ...
      if (circuit.tapWatts > 0 && !tapExists && speaker.hiZTaps.isNotEmpty) {
        throw InvalidCircuitException(
          ...
          context: {
            'available_taps': speaker.hiZTaps,
  ...
  static void validateImpedance(Circuit circuit, double totalImpedance, Speaker speaker) {
  ...
  static AmpMatchingException wrapException(Exception originalException, String step, [Map<String, dynamic>? context]) {
    ...
    return AmpMatchingException(
      'Unexpected error during $step: ${originalException.toString()}',
      step: step,
      context: context,
    );
  }
  ...
  static ValidationResult validateFinalResults(AmpMatchingResult result, List<Circuit> originalCircuits) {
    ...
    return ValidationResult(
      ...
      warnings: warnings,
      metadata: {
        'circuits_processed': originalCircuits.length,
        'circuits_assigned': assignedCircuits.length,
        'amplifiers_used': result.assignments.length,
      },
    );
  }
}

...
Future<void> demonstrateAmplifierMatching() async {
  ...
  try {
    ...
    for (int i = 0; i < result.assignments.length; i++) {
      ...
      print('  • ${assignment.ampModel.name}: ${assignment.circuits.length}/${assignment.ampModel.channels} channels (${(assignment.channelUtilization * 100).toStringAsFixed(1)}% utilization)');

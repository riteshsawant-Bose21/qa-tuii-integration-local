...
{
	TRTP_stream_info* pRTP_stream_info = &self->m_tRTPStream.m_RTP_stream_info;
	fusion_aes67_netfilter* pEth_netfilter = self->m_tRTPStream.m_pEth_netfilter;
	rtp_audio_stream_ops* pManager = self->m_pManager;
/*
	if(!self->m_bLivesInitialized)
	{
...
*/
	...
	while(ui32NbOfSampleRemaining > 0)
	{
		...
		if(acquire_tx_packet(pEth_netfilter, &pHandle, &pvPacket, &ulPacketSize) && ulPacketSize >= ulMinPacketSize)
		{
			...
			tx_acquired_packet(pEth_netfilter, pHandle, pvPacket, ui32PacketSize);
			MTAL_RtTraceEvent(RTTRACEEVENT_RTP_OUT, (PVOID)(RT_TRACE_EVENT_SIGNAL_STOP), 0);
		...
		else
		{
			...
			if(pHandle)
			{
				tx_acquired_packet(pEth_netfilter, pHandle, pvPacket, 0);
			}

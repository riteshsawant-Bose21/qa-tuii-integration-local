    ...
    if (ep->has_i2c && ep->i2c_client) {
        ...
        for (i = 0; i < cmd->num_i2c_cmds; ++i) {
            ...
            if (ret < 0) {
              ...
              //  dev_info(&bd_drvdata->pdev->dev, "Wrote register 0x%04x with data 0x%04x to I2C device at 0x%02x\n",
                                                                           // data->reg_addr, data->data_mask, client->addr);
            }
        }
    }
...
static int set_linked_or_aggregate_gpio(struct endpoint_gpio *ep_gpio, int num_gpios, struct endpoint_gpio *parent_gpios) 
{
    ...
    if (ep_gpio->type == EP_GPIO_TYPE_VIRT) {
        ...
        if (parent_ic && !ep_gpio->is_irq) {
            ...
            for (i = 0; i < parent_ic->num_eps; ++i) {
                ...
                ret = set_linked_or_aggregate_gpio(ep_gpio, ep->num_gpios, ep->gpios);
                if (!ret) {
                    return;
                }
            }
        ...
        for (j = 0; j < ep->num_gpios; ++j) {
            ...
            if (ep->type >= EP_TYPE_IOEXP_START && ep->type < EP_TYPE_IOEXP_END) {
                if (ep_gpio->ioexp_id != ep->ioexp_id) {
                    dev_err(&bd_drvdata->pdev->dev, "GPIO %s:%s doesn't have matching ioexp_id (%d/%d)\n", ep->name, 
                                                                                                ep_gpio->name, 
                                                                                                ep_gpio->ioexp_id, 
                                                                                                ep->ioexp_id);
                    continue;
...
static int configure_io_card_gpios(struct io_card *ic) 
{
    ...
    for (i = 0; i < ic->num_eps; ++i) {
        ...
        for (j = 0; j < ep->num_gpios; ++j) {
            ...
            if (ep->type >= EP_TYPE_IOEXP_START && ep->type < EP_TYPE_IOEXP_END) {
                if (ep_gpio->ioexp_id != ep->ioexp_id) {
                    dev_err(&bd_drvdata->pdev->dev, "GPIO %s:%s:%s doesn't have matching ioexp_id (%d/%d)\n", ic->data.model,
                                                                                                ep->name,
                                                                                                ep_gpio->name,  
                                                                                                ep_gpio->ioexp_id, 
                                                                                                ep->ioexp_id);

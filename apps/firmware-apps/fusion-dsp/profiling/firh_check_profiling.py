#!/usr/bin/env python3
"""
Profiling results checker for FIR Hybrid.
...
def check_profiling_results():
    """Load and display profiling results with pass/fail status."""
    
    try:
        df = pd.read_csv('../profiling_results/fir_hybrid/fir_hybrid_timings.csv')
    except FileNotFoundError:
        print("Error: profiling_results/fir_hybrid/fir_hybrid_timings.csv not found")
        print("Run profiling first: cd profiling && python profile_block.py fir_hybrid --local")
        return 1
    
    print("=" * 70)
    print(" FIR HYBRID PROFILING - QUICK CHECKER")
    print("=" * 70)
    print(f"\nTarget: {FRAME_TIME*1000:.3f} ms per frame | MIPS Budget: {TARGET_MIPS}%")
    print(f"Total samples: {len(df):,}\n")
    
    summary = df.groupby(['channels', 'num_taps'])['fir_hybrid'].agg(['mean', 'max'])

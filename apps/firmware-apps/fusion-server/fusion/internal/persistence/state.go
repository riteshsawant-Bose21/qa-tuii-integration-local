...
func (sm *StateManager) validateState(list *memberlist.Memberlist) {
	...
	for _, member := range members {
		...
		url := fmt.Sprintf("http://%s:%s%s", member.Addr.String(), api.AdminPort, routes.StateEndpoint)
...
func (sm *StateManager) getMemberData(list *memberlist.Memberlist) []api.MemberMetadata {
	...
	for _, member := range members {
		...
		url := fmt.Sprintf("http://%s:%s%s", member.Addr.String(), api.HTTPPort, routes.MetadataEndpoint)
		resp, err := http.Get(url)
		if err != nil {
...
func (sm *StateManager) validateData(list *memberlist.Memberlist) {
	...
	exportURL := fmt.Sprintf("http://%s:%s%s", mostCurrent.Member.Addr.String(), api.AdminPort, routes.StateEndpoint)
...
func syncData(memberMetadata []api.MemberMetadata, currentHash string, data []byte) {
	for _, ms := range memberMetadata {
		if ms.Metadata.Hash != currentHash {
			importURL := fmt.Sprintf("http://%s:%s%s", ms.Member.Addr.String(), api.AdminPort, routes.DataEndpoint)

...
private:
  ...
  void handleValueChange(const std::string &path, const Json::Value &old_val,
    ...
    if (old_val != new_val) {
      ...
      {
          rx_message += "[";
          for (uint32_t idx=0; idx < new_val.size()-1; idx++)
          {
              rx_message += new_val[idx].asString();
              rx_message += ",";
          }
          rx_message += new_val[new_val.size()-1].asString();
          rx_message += "]";
      }
      else if (new_val.isInt())
      {
          rx_message += new_val.asString();
      }
      else
      {
          // String
  ...
  void receiveLoop() {
      ...
      struct pollfd obsPollFd;

      obsPollFd.fd = sockfd;
      obsPollFd.events = POLLIN;

...
public:
  static constexpr std::size_t META_DATA_SHM_LENGTH =
      Metadata::META_DATA_MAX_SIZE + sizeof(pthread_mutex_t);
  ...
  /**
   * Reads data and updates metadata from the shared memory in a single
   * operation.
  ...
  std::size_t readFullStateFromSharedMemory(void *buffer,
  ...
  /**
   * Reads totalBytesWritten from the metadata in shared memory and updates the
   * relevant member.
   */
...
private:
  ...
  boost::interprocess::mapped_region
      region_; // Boost mapped region for accessing memory

  boost::interprocess::shared_memory_object
      metadataShm_;                                   // Metadata shared memory
  boost::interprocess::mapped_region metadataRegion_; // Metadata mapped region
                                                      //
  pthread_mutex_t *sharedMutex_;                      // Process-shared mutex
                                                      //
  std::atomic<bool> isReaderObject; // NamedSharedMemory can have personality
                                    // of producer or consumer at a given time
                                    // (not both)

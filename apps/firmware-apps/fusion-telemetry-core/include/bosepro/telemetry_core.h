...
// Describes if subscriber is internal (unix socket) or external (internet
// socket)
enum eSockConnType {
...
struct HandlerContext {
  eMeterCategory meter_type = TELM_METER_CTGRY_MAX;
};

// REQUEST Message handler prototype
typedef int (*telmReqProto)(telemetryManager &,
                            const bosepro::Telemetry_configuration &,
                            uint64_t &, std::string &, HandlerContext &ctx);

// RESPONSE Message handler prototype
typedef int (*telmRspProto)(telemetryManager &, std::string &, uint64_t, bool,
                            std::ostringstream &, HandlerContext &ctx);
...
public:
  ...
  std::size_t read_meter_data(enum eMeterCategory type,
    ...
    try {
      ...
      try {
        ...
        size = shared_mem[type].handle->read(static_cast<void *>(buf_ptr),
  ...
  ///
  /// @param  type  Meter type
  void reset_report_tick(enum eMeterCategory type) { report_tick[type] = 0; }

  uint32_t increment_fail_comm() { return ++comm_fail_count; }

  void reset_fail_comm() { comm_fail_count = 0; }

  uint32_t get_comm_fail_cnt() { return comm_fail_count; }
...
class telemetrySubscriberZmqInterface
    : public telemetrySubscriberInterfaceBase {
public:
  telemetrySubscriberZmqInterface() : telm_zmq_context(1) {}

  ~telemetrySubscriberZmqInterface() { telm_zmq_socket.close(); }
  ...
  int send(const std::string &pub_data) override {
    ...
    try {
      auto send_res = telm_zmq_socket.send(msg, zmq::send_flags::none);
      if (send_res.has_value()) {
        SPDLOG_DEBUG("Message sent successfully ({} bytes)", send_res.value());
      } else {
...
public:
  ...
  int init(std::string &core_ip, std::string &core_path,
    ...
    if (ret_val == 0) {
      ...
      SPDLOG_DEBUG("Update periods: {} {} {}", hi_update_period_frames,
                   med_update_period_frames, lo_update_period_frames);
    }
    ...
    if (ret_val == 0) {
      ...
      SPDLOG_DEBUG("Report periods: {} {} {}", hi_report_period_frames,
                   med_report_period_frames, lo_report_period_frames);
    }
  ...
  int register_publisher(std::string pub_name,
    ...
    std::map<std::string, std::unique_ptr<telemetryPublisher>>::iterator pub_it;
    ...
    if (pub_it == publishers.end()) {
      ...
      for (int idx = 0; idx < 3; idx++) {
        if (shm_config[idx].size > 0) {
          ...
          create_shared_mem(shm_config[idx].name, shm_config[idx].size);
          shm_config[idx].handle = &(get_shared_mem(shm_config[idx].name));
      ...
      auto new_entry = publishers.emplace(
          pub_name,
          std::make_unique<telemetryPublisher>(pub_name, pub_addr, shm_config));
  ...
  int deregister_publisher(std::string pub_name) {
    ...
    if (pub_it != publishers.end()) {
      if (pub_it->second->get_shared_mem_size(TELM_METER_CTGRY_HI_PRIO) > 0) {
        ...
        pub_shm_name.assign(
            pub_it->second->get_shared_mem_name(TELM_METER_CTGRY_HI_PRIO));
        remove_shared_mem(pub_shm_name);
      }
      if (pub_it->second->get_shared_mem_size(TELM_METER_CTGRY_MED_PRIO) > 0) {
        std::string pub_shm_name;
        pub_shm_name.assign(
            pub_it->second->get_shared_mem_name(TELM_METER_CTGRY_MED_PRIO));
        remove_shared_mem(pub_shm_name);
      }
      if (pub_it->second->get_shared_mem_size(TELM_METER_CTGRY_LO_PRIO) > 0) {
        std::string pub_shm_name;
        pub_shm_name.assign(
            pub_it->second->get_shared_mem_name(TELM_METER_CTGRY_LO_PRIO));
        remove_shared_mem(pub_shm_name);
      }
  ...
  int set_meter_report_periods(std::vector<int_fast32_t> &periods) {
    ...
    // Validate periods
    if ((periods.size() <= 0) || (periods.size() > TELM_METER_CTGRY_MAX) ||
      ...
      if (((periods[TELM_METER_CTGRY_HI_PRIO] % hi_update_period_frames) !=
           0) ||
          ((periods[TELM_METER_CTGRY_MED_PRIO] % med_update_period_frames) !=
           0) ||
          ((periods[TELM_METER_CTGRY_LO_PRIO] % lo_update_period_frames) !=
           0)) {
        ...
        report_period_factor[TELM_METER_CTGRY_HI_PRIO] =
            periods[TELM_METER_CTGRY_HI_PRIO] / hi_update_period_frames;
  ...
  void get_meter_report_periods(uint32_t &hi_period, uint32_t &med_period,
                                uint32_t &lo_period) {
  ...
  void get_meter_update_periods(uint32_t &hi_period, uint32_t &med_period,
                                uint32_t &lo_period) {
  ...
  void receive_data(enum eSockConnType type, char rx_buf[], int rx_buf_size,
                    ssize_t &recv_size) {
    clear_last_rcvd_addr();
    if (type == TELM_CONN_TYPE_UNIX_SOCK) {
      socklen_t addr_length = sizeof(ux_addr_last_rcvd);

      recv_size = recvfrom(socket_fd[TELM_CONN_TYPE_UNIX_SOCK], (void *)rx_buf,
                           rx_buf_size, 0,
                           (struct sockaddr *)&ux_addr_last_rcvd, &addr_length);
    }

    else {
      recv_size = recvfrom(socket_fd[TELM_CONN_TYPE_INTERNET_SOCK],
                           (void *)rx_buf, rx_buf_size, 0, NULL, NULL);
    }
  }
  ...
  int register_message_handler(const std::string &message_name,
    ...
    if (init_success) {
      if (message_handler.emplace(message_name, telmPairMsgHdlr(req, rsp))
              .second) {
        ret_val = 0;
      } else {
        // Handler already registered
        SPDLOG_ERROR("Handlers already registered - {}", message_name);
        ret_val = -1;
      }
    } else {
  ...
  /// @return Success(0)/Failure (-1)
  int send_data(const std::string destination_name,
  ...
  void find_type(std::string req_name, enum etelemetryEndpointTypes &type) {
  ...
  void get_ux_sock_path(std::string &ux_path) { ux_path = core_sock_path; }

  /// Get the IP address of system
  ///
  /// @param  ip_addr  IP address
  void get_inet_ip(std::string &ip_addr) { ip_addr = core_ip_addr; }

  /// Get IP Port number of system
  ///
  /// @param ip_port   Port number of system
  void get_inet_port(uint32_t &ip_port) { ip_port = core_ip_port; }
  ...
  int get_socket_descriptor(enum eSockConnType type) { return socket_fd[type]; }
  ...
  std::size_t get_meter_data(std::string pub_name,
    ...
    if (pub_it != publishers.end()) {
      data_size = pub_it->second->read_meter_data(meter_type, meter_data);
    } else {
      SPDLOG_ERROR("Publisher not found ({})", pub_name);
    }
...
private:
  ...
  struct tx_data {
    ...
    tx_data(int *fd, const std::string &message) : fd_(fd), in_msg_(message) {}
    ...
    int operator()(sockaddr_un addr) {
      ...
      if (stat(addr.sun_path, &statbuf) == 0 && S_ISSOCK(statbuf.st_mode)) {
        return sendto(fd_[TELM_CONN_TYPE_UNIX_SOCK], tx_stream,
                      in_msg_.length(), 0, (struct sockaddr *)&addr,
                      sizeof(addr));
      } else {
    ...
    int operator()(sockaddr_in addr) {
      ...
      return sendto(fd_[TELM_CONN_TYPE_INTERNET_SOCK], tx_stream,
                    in_msg_.length(), 0, (struct sockaddr *)&addr,
                    sizeof(addr));
    }
  };
  ...
  void meter_update_req_add(uint64_t &pkt_id, const std::string &pub_name,
  ...
  uint32_t meter_update_req_remove(uint64_t &pkt_id) {
    return meter_update_tracker.erase(pkt_id);
  }

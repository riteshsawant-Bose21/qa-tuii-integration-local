...
static int ppoll(struct pollfd *fds, nfds_t nfds,
  ...
  if (timeout_ts != nullptr) {
    ...
    const long long ms_from_sec =
        static_cast<long long>(timeout_ts->tv_sec) * 1000LL;
    const long long ms_from_nsec = timeout_ts->tv_nsec / 1000000LL;
    const long long total_ms = ms_from_sec + ms_from_nsec;
    timeout_ms = (total_ms > static_cast<long long>(INT_MAX))
                     ? INT_MAX
...
int register_message_handlers(bosepro::telemetryManager &telm_mgr) {
  ...
  ret_val = telm_mgr.register_message_handler(
      "pub_register_req", process_pub_register_req, process_pub_register_rsp);
  ...
  ret_val = telm_mgr.register_message_handler(
      "update_meters_rsp", process_update_meters_rsp, process_meter_data);
  if (ret_val != 0) {
    return ret_val;
  }
  ...
  ret_val = telm_mgr.register_message_handler(
      "send_meter_req", process_send_meter_req, process_send_meter_rsp);
  ...
  // Message: event
  ret_val = telm_mgr.register_message_handler("event", process_event,
                                              process_event_rsp);
  ...
  ret_val = telm_mgr.register_message_handler(
      "report_period", process_update_report_period_req, NULL);
  return ret_val;
}
...
void handle_update(const std::string &update_setting) {
  ...
  snprintf(core_addr_obs.sun_path, (core_path_for_obsvr.size() + 1), "%s",
           core_path_for_obsvr.c_str());

  if (observer_sock_fd != -1) {
    sendto(observer_sock_fd, update_setting.data(), update_setting.size(), 0,
           (struct sockaddr *)&core_addr_obs, sizeof(core_addr_obs));
...
int main(int argc, char *argv[]) {
  ...
  // Note: on actual devices CONFIG_PATH will be set appropriately. And shared
  // configuration like telemetry-configuration.json will have symlinks to the
  // "real" file.
  std::string config_path =
      "config"; // Default config path for local testing, etc.
  ...
  desc.add_options()(
      ...
      boost::program_options::value<std::string>()->default_value(
          config_path + "/telemetry-configuration.json"),
      "configuration file")(
      "socket-path,p", boost::program_options::value<std::string>(&core_path),
      "Core UNIX Domain Socket Path")(
      "system-ip,i",
      boost::program_options::value<std::string>(&core_ip)->required(),
      "System IP Address")(
      "update-period,u",
      boost::program_options::value<std::vector<uint32_t>>(&update_periods)
          ->multitoken(),
      "HI Freq., MED Freq. & LO Freq. update periods (Frames)")(
      "report-period,r",
      boost::program_options::value<std::vector<uint32_t>>(&report_periods)
          ->multitoken(),
      "HI Freq., MED Freq. & LO Freq. report periods (Frames)")(
      "debug-log-enable,d", boost::program_options::bool_switch(),
      "Set logging level to DEBUG")("help,h", "print this message and exit");
  ...
  bosepro::Telemetry_configuration configuration(
      vm["configuration"].as<std::string>());
  bosepro::Telemetry_configuration telm_config(
      configuration.get_telemetry_configuration());
  ...
  // Calibrate timeout value (~2/3 ms)
  {
    ...
    SPDLOG_INFO("Calibration Complete (Ref time: {}({}))", calibrated_period_ns,
                end);
  }
  ...
  client = new UDPValueMonitor("127.0.0.1", fus_serv_port, target_paths,
                               handle_update);

  // Initialize manager
  if (telmMgr->init(core_ip, core_path, core_pub_addr, update_periods,
                    report_periods) != 0) {
  ...
  while (telm_running) {
    ...
    poll_ret = ppoll(telmPollFds, TELM_CONN_TYPE_MAX, &poll_timeout, &sigmask);

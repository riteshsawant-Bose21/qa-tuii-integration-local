...
bosepro::NamedSharedMemory::NamedSharedMemory(const char* name, std::size_t size, bool create)
    ...
    try {
        ...
        if (create) {
            ...
            metadataShm_.truncate(sizeof(pthread_mutex_t) + sizeof(std::size_t) * 3 + sizeof(bosepro::WriteBlock) * Metadata::MAX_WRITE_BLOCKS);
        } else {
 ...
 */
void accessSharedMemoryWithMutex(bosepro::NamedSharedMemory& sharedMemory, const std::function<void()>& action) {

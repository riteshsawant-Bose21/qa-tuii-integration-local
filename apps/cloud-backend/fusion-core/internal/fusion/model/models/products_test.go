...
var (
	productDBTypes = map[string]string{`ID`: `integer`, `Category`: `text`, `Description`: `text`, `Price`: `numeric`, `MetaInfo`: `jsonb`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_              = bytes.MinRead
)

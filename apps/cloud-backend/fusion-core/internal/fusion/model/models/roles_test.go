...
func testRolesInsertWhitelist(t *testing.T) {
	...
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	if err = o.Insert(ctx, tx, boil.Whitelist(strmangle.SetMerge(rolePrimaryKeyColumns, roleColumnsWithoutDefault)...)); err != nil {
		t.Error(err)
	}
...
func testRoleToManyAccountTypeRoles(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := RoleSlice{&a}
	if err = a.L.LoadAccountTypeRoles(ctx, tx, false, (*[]*Role)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testRoleToManyFeaturePermissions(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := RoleSlice{&a}
	if err = a.L.LoadFeaturePermissions(ctx, tx, false, (*[]*Role)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testRoleToManyAddOpAccountTypeRoles(t *testing.T) {
	var err error
...
func testRoleToManyAddOpFeaturePermissions(t *testing.T) {
	var err error
...
var (
	roleDBTypes = map[string]string{`ID`: `integer`, `Name`: `character varying`, `Description`: `text`}
	_           = bytes.MinRead
)

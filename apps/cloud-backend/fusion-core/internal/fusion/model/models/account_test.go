...
func testAccountToManyAccountTypeRoles(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = a.L.LoadAccountTypeRoles(ctx, tx, false, (*[]*Account)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testAccountToManyUsers(t *testing.T) {
	...
	slice := AccountSlice{&a}
	if err = a.L.LoadUsers(ctx, tx, false, (*[]*Account)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testAccountToManyAddOpAccountTypeRoles(t *testing.T) {
	var err error
...
var (
	accountDBTypes = map[string]string{`ID`: `integer`, `Name`: `character varying`, `Description`: `character varying`, `Type`: `enum.accounts_type_enum('admin','reseller','retail')`, `CreatedAt`: `timestamp without time zone`}
	_              = bytes.MinRead
)

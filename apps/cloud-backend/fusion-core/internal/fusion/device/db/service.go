...
func (s *Service) insertOwnershipHistory(ctx context.Context, deviceUUID, accountID, certID, certArn string, tx model.DBTxExecutor, logger *zap.Logger) error {
	if deviceUUID == "" {
		return fmt.Errorf("insertOwnershipHistory: %s", errorutil.ErrMsgDeviceUUIDEmpty)
	}
	if accountID == "" {
		return fmt.Errorf("insertOwnershipHistory: %s", errorutil.ErrMsgAccountIDEmpty)
	}
	if certID == "" {
	...
	if err := history.Insert(ctx, tx, boil.Infer()); err != nil {
		...
		return fmt.Errorf("insertOwnershipHistory: failed to insert ownership history for device %s: %w", deviceUUID, err)
	}
	return nil
}
...
func (s *Service) insertProjectHistory(ctx context.Context, deviceUUID, projectID string, tx model.DBTxExecutor, logger *zap.Logger) error {
	if deviceUUID == "" {
		return fmt.Errorf("insertProjectHistory: %s", errorutil.ErrMsgDeviceUUIDEmpty)
	}
	if projectID == "" {
		return fmt.Errorf("insertProjectHistory: %s", errorutil.ErrMsgProjectIDEmpty)
	}
	...
	if err := history.Insert(ctx, tx, boil.Infer()); err != nil {
		...
		return fmt.Errorf("insertProjectHistory: failed to insert project history for device %s project %s: %w", deviceUUID, projectID, err)
	}
	return nil
}
...
func (s *Service) decommissionCurrentProject(ctx context.Context, deviceUUID, projectID string, tx model.DBTxExecutor, logger *zap.Logger) error {
	if deviceUUID == "" {
		return fmt.Errorf("decommissionCurrentProject: %s", errorutil.ErrMsgDeviceUUIDEmpty)
	}
	if projectID == "" {
		return fmt.Errorf("decommissionCurrentProject: %s", errorutil.ErrMsgProjectIDEmpty)
	}
	...
	if err != nil {
		logger.Error("Failed to get device project history",
			...
			zap.Error(err))
		return fmt.Errorf("decommissionCurrentProject: failed to get device project history for device %s project %s: %w", deviceUUID, projectID, err)
	}
	...
	if _, err = history.Update(ctx, tx, boil.Infer()); err != nil {
		...
		return fmt.Errorf("decommissionCurrentProject: failed to decommission project history for device %s project %s: %w", deviceUUID, projectID, err)
	}
	return nil
}
...
func (s *Service) Insert(ctx context.Context, req *types.DeviceCreateRequest, accountID string, cert types.CertificateInfo, tx model.DBTxExecutor, logger *zap.Logger) error {
	...
	if req == nil {
		return fmt.Errorf("Insert: %s", errorutil.ErrMsgDeviceCreateReqNil)
	}
	if accountID == "" {
		return fmt.Errorf("Insert: %s", errorutil.ErrMsgAccountIDEmpty)
	}
	...
	if err := device.Insert(ctx, tx, boil.Infer()); err != nil {
		if strings.Contains(err.Error(), "violates unique constraint") {
			return fmt.Errorf("Insert: %w", errorutil.ErrDeviceUniqueConstraint)
		}
		logger.Error("Failed to insert device", zap.String("deviceID", req.SerialNumber), zap.Error(err))
		return fmt.Errorf("Insert: failed to insert device %s: %w", req.SerialNumber, err)
	}

	// Create ownership history
	if err := s.insertOwnershipHistory(ctx, device.ID, accountID, cert.ID, cert.Arn, tx, logger); err != nil {
		return fmt.Errorf("Insert: failed to insert ownership history for device %s: %w", req.SerialNumber, err)
	}

	// Create project history
	if err := s.insertProjectHistory(ctx, device.ID, req.ProjectID, tx, logger); err != nil {
		return fmt.Errorf("Insert: failed to insert project history for device %s: %w", req.SerialNumber, err)
	}
...
func (s *Service) Update(ctx context.Context, device models.Device, req *types.DeviceUpdateRequest, tx model.DBTxExecutor, logger *zap.Logger) error {
	if req == nil {
		return fmt.Errorf("Update: %s", errorutil.ErrMsgDeviceUpdateReqNil)
	}
	...
	if _, err := device.Update(ctx, tx, boil.Infer()); err != nil {
		...
		return fmt.Errorf("Update: failed to update device %s: %w", device.SerialNumber, err)
	}
...
func (s *Service) handleProjectChange(ctx context.Context, device *models.Device, newProjectID string, tx model.DBTxExecutor, logger *zap.Logger) error {
	...
	if err := s.decommissionCurrentProject(ctx, device.ID, device.ProjectID.String, tx, logger); err != nil {
		return fmt.Errorf("handleProjectChange: failed to decommission current project for device %s: %w", device.SerialNumber, err)
	}
...
func (s *Service) Reset(ctx context.Context, device models.Device, tx model.DBTxExecutor, logger *zap.Logger) error {
	...
	if err != nil {
		...
		return fmt.Errorf("Reset: failed to get ownership history for device %s: %w", device.SerialNumber, err)
	}
	...
	if _, err = ownerHistory.Update(ctx, tx, boil.Infer()); err != nil {
		...
		return fmt.Errorf("Reset: failed to update ownership history for device %s: %w", device.SerialNumber, err)
	}

	// Decommission current project
	if err := s.decommissionCurrentProject(ctx, device.ID, device.ProjectID.String, tx, logger); err != nil {
		return fmt.Errorf("Reset: failed to decommission current project for device %s: %w", device.SerialNumber, err)
	}
	...
	if _, err = device.Update(ctx, tx, boil.Infer()); err != nil {
		...
		return fmt.Errorf("Reset: failed to reset device %s: %w", device.SerialNumber, err)
	}
...
func (s *Service) ClaimDevice(ctx context.Context, device models.Device, accountID string, cert types.CertificateInfo, req *types.DeviceClaimRequest, tx model.DBTxExecutor, logger *zap.Logger) error {
	if accountID == "" {
		return fmt.Errorf("ClaimDevice: %s", errorutil.ErrMsgAccountIDEmpty)
	}

	if req == nil {
		return fmt.Errorf("ClaimDevice: %s", errorutil.ErrMsgDeviceRequestInvalid)
	}

	if req.ProjectID == "" {
		return fmt.Errorf("ClaimDevice: %s", errorutil.ErrMsgProjectIDEmpty)
	}
	...
	if _, err := device.Update(ctx, tx, boil.Infer()); err != nil {
		if strings.Contains(err.Error(), "violates unique constraint") {
			return fmt.Errorf("ClaimDevice: %w", errorutil.ErrDeviceUniqueConstraint)
		}
		logger.Error("Failed to claim device", zap.String("serialNumber", device.SerialNumber), zap.Error(err))
		return fmt.Errorf("ClaimDevice: failed to claim device %s: %w", device.SerialNumber, err)
	}

	// Create ownership history
	if err := s.insertOwnershipHistory(ctx, device.ID, accountID, cert.ID, cert.Arn, tx, logger); err != nil {
		return fmt.Errorf("ClaimDevice: failed to insert ownership history for device %s: %w", device.SerialNumber, err)
	}

	// Create project history
	if err := s.insertProjectHistory(ctx, device.ID, req.ProjectID, tx, logger); err != nil {
		return fmt.Errorf("ClaimDevice: failed to insert project history for device %s: %w", device.SerialNumber, err)
	}
...
func (s *Service) UpdateCertificate(ctx context.Context, device models.Device, cert types.CertificateInfo, tx model.DBTxExecutor, logger *zap.Logger) error {
	...
	if _, err := device.Update(ctx, tx, boil.Infer()); err != nil {
		...
		return fmt.Errorf("UpdateCertificate: failed to update certificate for device %s: %w", device.SerialNumber, err)
	}
...
func (s *Service) InsertCommand(ctx context.Context, projectID, commandID string, request *types.CommandRequest, logger *zap.Logger) error {
	if projectID == "" {
		return fmt.Errorf("InsertCommand: %s", errorutil.ErrMsgProjectIDEmpty)
	}
	if commandID == "" {
		return fmt.Errorf("InsertCommand: %s", errorutil.ErrMsgCommandIDEmpty)
	}
	if request == nil {
		return fmt.Errorf("InsertCommand: %s", errorutil.ErrMsgCommandReqNil)
	}
	...
	if len(request.DeviceIDs) == 0 {
		...
		if err := command.Insert(ctx, s.db, boil.Infer()); err != nil {
			...
			return fmt.Errorf("InsertCommand: failed to insert command %s for project %s: %w", commandID, projectID, err)
		}
	}
	...
	for _, deviceID := range request.DeviceIDs {
		...
		if err != nil {
			logger.Error("Failed to get device by ID", zap.String("deviceID", deviceID), zap.Error(err))
			return fmt.Errorf("InsertCommand: %w", err)
		}
		...
		if err := command.Insert(ctx, s.db, boil.Infer()); err != nil {
			...
			return fmt.Errorf("InsertCommand: failed to insert command %s for device %s: %w", commandID, deviceID, err)
		}
	}
	return nil
}
...
func (s *Service) UpdateCommandStatus(ctx context.Context, commandID string, status string, logger *zap.Logger) error {
	if commandID == "" {
		return fmt.Errorf("UpdateCommandStatus: %s", errorutil.ErrMsgCommandIDEmpty)
	}
	if status == "" {
		return fmt.Errorf("UpdateCommandStatus: %s", errorutil.ErrMsgStatusEmpty)
	}
	...
	if err != nil {
		...
		return fmt.Errorf("UpdateCommandStatus: failed to get command history for %s: %w", commandID, err)
	}
	...
	for _, command := range commands {
		...
		if _, err := command.Update(ctx, s.db, boil.Infer()); err != nil {
			...
			return fmt.Errorf("UpdateCommandStatus: failed to update command status for %s: %w", commandID, err)
		}
	}
...
func (s *Service) GetCommandStatus(ctx context.Context, commandID string, logger *zap.Logger) (*models.DeviceCommandHistorySlice, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("GetCommandStatus: failed to get command status for %s: %w", commandID, err)
	}
	return &commands, nil
}

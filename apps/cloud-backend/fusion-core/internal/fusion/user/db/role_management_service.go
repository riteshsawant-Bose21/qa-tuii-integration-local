...
func (s *RoleManagementService) GetOrganizationRoleManagement(ctx context.Context, accountID string) (*types.RoleManagementResponse, error) {
...
func (s *RoleManagementService) getRolesWithPermissions(ctx context.Context, accountID string) ([]types.RoleWithPermissions, error) {
	query := `
...
func (s *RoleManagementService) getRolePermissions(ctx context.Context, roleID int, accountID string) ([]types.FeaturePermissionDetail, error) {
	query := `
		SELECT 
...
func (s *RoleManagementService) getRoleUserCount(ctx context.Context, roleID int, accountID string) (int, error) {
	query := `
		SELECT COUNT(*)
		FROM app_user u
...
func (s *RoleManagementService) getAllFeatures(ctx context.Context) ([]types.Feature, error) {
...
func (s *RoleManagementService) getAllAccessLevels(ctx context.Context) ([]types.AccessLevel, error) {
...
func (s *RoleManagementService) getOrganizationUsers(ctx context.Context, accountID string) ([]types.UserBasicInfo, error) {
	query := `
...
func (s *RoleManagementService) CreateRole(ctx context.Context, accountID string, req *types.CreateRoleRequest) (*types.Role, error) {
	// Start transaction
	...
	`
	err = tx.QueryRowContext(ctx, roleQuery, req.Name, req.Description).Scan(
	...
	err = tx.QueryRowContext(ctx, accountTypeQuery, accountID).Scan(&accountTypeID)
	if err != nil {
	...
	atrQuery := `
		...
		VALUES ($1, $2)
	`
	_, err = tx.ExecContext(ctx, atrQuery, accountTypeID, role.ID)
	if err != nil {
...
func (s *RoleManagementService) UpdateUserRole(ctx context.Context, userID, accountID string, newRoleID int) error {
...
func (s *RoleManagementService) UpdateRolePermissions(ctx context.Context, roleID int, accountID string, permissions []types.PermissionUpdateRequest) error {
	// Start transaction
	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
	...
	atrQuery := `
		...
		WHERE a.id = $1 AND atr.role_id = $2
	`
	err = tx.QueryRowContext(ctx, atrQuery, accountID, roleID).Scan(&accountTypeRoleID)
	if err != nil {
		return fmt.Errorf("failed to get account_type_role_id: %w", err)
	}
	...
	for _, perm := range permissions {
		...
		case "add", "update":
			...
			`
			_, err = tx.ExecContext(ctx, upsertQuery, perm.FeatureID, accountTypeRoleID, perm.AccessLevelID)
			if err != nil {
		...
		case "remove":
			...
			`
			_, err = tx.ExecContext(ctx, deleteQuery, perm.FeatureID, accountTypeRoleID)
			if err != nil {
...
func (s *RoleManagementService) CheckAdminPermission(ctx context.Context, userEmail, accountID string) (bool, error) {

...
import 'package:fusion_design_tool_prototype/core/models/listening_area_entity.dart';
      ...
      for (int i = 0; i < hardwareComponents.length; i++) {
        ...
        if (comp['componentType'] == 'source' ) {
          ...
          if (comp['type'] == 'aes67input') {
            ...
            comp['type'] == 'aes67output' && _isHardwareComponentInsideAnyZone(component)) {
          // Create output stream for AES67 outputs
          final String componentName =
              comp['name']?.toString() ?? comp['id']?.toString() ?? 'unknown';
  ...
  static String _extractServerLocation(Map<String, dynamic> component) {
    if (component['locationEntity'] != null) {
      ...
      if (location['floorId'] != null &&
          location['floorId'].toString().isNotEmpty) {
        ...
        floorName =
            projectManager.value.floors
                .firstWhere((Floor floor) => floor.id == floorId)
                .name;

      if (location['listeningAreaId'] != null &&
          location['listeningAreaId'].toString().isNotEmpty) {

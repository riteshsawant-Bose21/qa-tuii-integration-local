...
import '../../configuration_aes67/viewModel/input_stream_viewmodel/input_stream_viewmodel.dart';
import '../view_model/add_source_viewmodel.dart';
import 'widgets/add_sources_popup_content.dart';

/// Local enum to avoid cross-feature imports from the controller feature.
enum SourceLocationType { zone, equipmentLocation }

class AddSourceDrawer extends StatelessWidget {
  final Widget child;
  final void Function()? onSaved;
  final bool isFromBuildingPage;
  ...
  static Future<void> show(
    ...
    final ConfigAes67Viewmodel configAes67VM = ConfigAes67Viewmodel(projectViewModel: projectVM);
    final InputStreamViewmodel inputStreamVM = InputStreamViewmodel(projectViewModel: projectVM)..init();
    final AddSourceViewModel addSourceVM =
    ...
    bool _isAes67Complete(AddSourceViewModelState state) {
      ...
      if (state.assignedStreamChannels.isNotEmpty) return true;

      // Otherwise check the signal type dropdowns
      if (state.selectedSignalType == SignalType.mono) {
        return state.selectedMonoChannel != null;
      } else if (state.selectedSignalType == SignalType.stereo) {
        return state.selectedLeftChannel != null && state.selectedRightChannel != null;
      }
    ...
    await FusionDrawer.show<void>(
      ...
      onButtonPressed: () => addSourceVM.onSaveTap(context),
      content: MultiBlocProvider(
        providers: <BlocProvider<dynamic>>[
          BlocProvider<ConfigAes67Viewmodel>.value(value: configAes67VM),
          BlocProvider<InputStreamViewmodel>.value(value: inputStreamVM),
          BlocProvider<AddSourceViewModel>.value(value: addSourceVM),
        ],
        child: AddSourcesPopupContent(isFromBuildingPage: isFromBuildingPage),
      ),
    );

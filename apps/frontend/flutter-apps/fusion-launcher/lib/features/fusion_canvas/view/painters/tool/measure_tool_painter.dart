...
class MeasureToolPainter extends FusionBasePainter {
  ...
  void paint(Canvas canvas, Size size, FusionCanvasPainter painter) {
    if (state is IdleMeasureToolState) {
      // No drawing for idle state
      return;
    }

    if (state is DrawingMeasureToolState) {
      final DrawingMeasureToolState drawingState = state as DrawingMeasureToolState;
      final Offset start = drawingState.start;
      final Offset end = drawingState.end ?? cursor ?? start;

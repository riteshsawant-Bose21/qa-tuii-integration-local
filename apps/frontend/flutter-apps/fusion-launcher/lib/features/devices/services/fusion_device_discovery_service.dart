...
class FusionDeviceDiscoveryService {
  ...
  getAvailableDevicesOnNetwork({
    ...
    try {
      final ResponseCallback<model.DeviceListResponse> response =
          await networkClient.getProto<model.DeviceListResponse>(
  ...
  FusionNetworkDevice _toFusionNetworkDevice(model.DeviceInfo device) {
    ...
    final String fallbackModelName =
        serialNumber == '07323a09dabc1d39' ? 'XLRPAL' : 'FM8Y';
    ...
    return FusionNetworkDevice(
      ...
      name: device.name,
      modelName:
          device.modelName.isNotEmpty ? device.modelName : fallbackModelName,

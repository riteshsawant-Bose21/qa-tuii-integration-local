...
Widget _buildListeningAreaSelectionSection(
  ...
  String? zoneName(String areaId) {
    String? zoneName =
        projectViewModel.getZonesForListeningArea(areaId: areaId)?.name;
    if (zoneName == null || zoneName.trim().isEmpty) {
      zoneName =
          projectViewModel.getSubZoneForListeningArea(areaId: areaId)?.name;
    }
    return zoneName;
  }

  final CreateZoneViewModel createZoneViewModel =
      rootContextFromParent.read<CreateZoneViewModel>();

  void onListeningAreaTap(
    bool isAlreadySelected,
    ListeningArea area,
    StateSetter setPopupState,
  ) {
    final bool isCreatingSubZonesAlongSide =
        createZoneViewModel.isCreatingSubZonesAlongSide;

    if (isCreatingSubZonesAlongSide) {
      createZoneViewModel.updateSubzoneListeningArea(
        subZoneIndex!,
        area,
        isAlreadySelected,
      );
  ...
  return FusionArrowPopup(
    ...
    content: StatefulBuilder(
      builder: (BuildContext context, StateSetter setPopupState) {
        final List<ListeningArea> allListeningAreas =
            context.watch<ProjectViewModel>().getAllListeningAreas();
                          ...
                          children: <Widget>[
                            ...
                            SemanticHelper.button(
                              testId: SemanticHelper.createTestId(
                                ...
                                "select_listening_areas_close_button",
                              ),
                              child: MouseRegion(
                                ...
                                child: Column(
                                  children: <Widget>[
                                    ...List<Widget>.generate(
                                      ...
                                      (int index) {
                                        final ListeningArea area =
                                            allListeningAreas[index];
                                        ...
                                        if (isCreatingSubZonesAlongSide) {
                                          isAvailableToSelectOrDeselect =
                                              createZoneViewModel
                                                  .isListeningAreaSelectedForSubzone(
                                                    subZoneIndex!,
                                                    area.id,
                                                  );
                                        } else {
                                          isAvailableToSelectOrDeselect =
                                              createZoneViewModel
                                                  .isListeningAreaSelectedForZone(
                                                    area.id,
                                                  );
                                        }
    ...
    child: Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: rootContextFromParent.colorScheme.strokeLight,
          width: 1,
        ),
      ),
      child: BlocProvider<CreateZoneViewModel>.value(
          ...
          builder: (BuildContext context, CreateZoneViewModelState state) {
            ...
            final int totalListeningAreasSelected =
                isCreatingSubZonesAlongSide
                    ? createZoneViewModel.totalLiseningAreasSelectedForSubzone(
                      subZoneIndex!,
                    )
                ...
                children: <Widget>[
                  Expanded(
                    child: FusionAppText(
                      ...
                      text:
                          totalListeningAreasSelected == 0
                              ? 'Select listening areas'
                              : '$totalListeningAreasSelected listening areas selected',
                      style: context.textTheme.labelLarge?.copyWith(
                        fontSize: 12,
                        color:
                            totalListeningAreasSelected == 0
                                ? context.colorScheme.onSurface.withAlpha(155)

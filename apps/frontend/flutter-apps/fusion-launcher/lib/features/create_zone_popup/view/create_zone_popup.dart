...
class CreateZonePopup extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTap: () => CreateZonePopup.show(context, isFromBuildingPage: isFromBuildingPage),
      child: child,
    );
  }
}

class FusionDrawerHeader extends StatefulWidget {
  const FusionDrawerHeader({
    super.key,
  ...
  /// Optional widget placed between the title and the close icon.
  final Widget? trailing;

  /// Called when the close icon is tapped. Defaults to Navigator.maybePop.
  final VoidCallback? onClose;

  @override
  State<FusionDrawerHeader> createState() => _FusionDrawerHeaderState();
}

class _FusionDrawerHeaderState extends State<FusionDrawerHeader> {
  bool _hovered = false;
  @override
  Widget build(BuildContext context) {
    return SemanticHelper.container(
      testId: SemanticHelper.createTestId(SemanticTypes.container, '${widget.semanticId}_drawer_header'),
      child: Padding(
          ...
          children: <Widget>[
            ...
            Expanded(
              child: FusionAppText(
                ...
                semanticId: '${widget.semanticId}_drawer_header_title',
            ...
            // Close icon
            SemanticHelper.button(
              testId: SemanticHelper.createTestId(SemanticTypes.button, '${widget.semanticId}_drawer_close_icon'),
              child: MouseRegion(
                ...
                child: GestureDetector(
                  ...
                  child: FusionIcon.icon(
                    semanticId: '${widget.semanticId}_drawer_close_icon',
                    Icons.close,
                    size: 16,
                    color: _hovered ? context.colorScheme.elevation6 : context.colorScheme.iconWhite,
                  ),
                ),
              ),
            ),

...
class MessageVcPanel extends StatelessWidget {
  ...
  Widget _buildContent(BuildContext context, MessageLoaded state) {
    ...
    final Set<String> checkedIds = activePlayer != null ? state.getSelectedMessagesForPlayer(activePlayer.id) : <String>{};

    final List<MessageModel> checkedMessages =
        activePlayer != null ? state.getMessagesForPlayer(activePlayer.id).where((MessageModel m) => checkedIds.contains(m.id)).toList() : <MessageModel>[];
                ...
                Flexible(
                  child: ListView.builder(
                    ...
                    itemBuilder: (BuildContext context, int index) {
                      return _MessageVcItem(message: checkedMessages[index]);
                    },
                  ),
                ),
            ],
    ...
    return Container(
      ...
      child: Row(
        children: <Widget>[
          ...
          Icon(Icons.chevron_right, size: 18, color: context.colorScheme.iconDefault),
        ],
      ),
    );
  }

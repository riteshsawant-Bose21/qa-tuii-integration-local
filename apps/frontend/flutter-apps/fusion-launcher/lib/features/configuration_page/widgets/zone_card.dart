...
class _ZoneCardState extends State<ZoneCard> {
          ...
          itemBuilder: (BuildContext context, int index) {
            ...
            return ReorderableDragStartListener(
              ...
              child: Padding(
                ...
                child: buildPriorityFunctionWidget(
  ...
  Widget buildPriorityFunctionWidget({required int priorityIndex}) {
    ...
    if (priorityIndex == 1) {
      if (prioritySources.isNotEmpty && prioritySources[0].isNotEmpty) {
        final HardwareComponent? sourceData = _projectViewModel.getHardware(
                                ...
                                child: Checkbox(
                                  ...
                                  onChanged:
                                      isPrioritySource
                                          ? null
                                          : (bool? _) {
                                            if (tempSelectedSources.contains(
                                              id,
                                            )) {
                                              tempSelectedSources.remove(id);
                                            } else {
  ...
  Widget _buildSubZonePanel(List<SubZone> subZonesForZone) {
    final List<CircuitModel> zoneCircuit = _projectViewModel.getCircuitsInZone(
      widget.zoneId,
    );
                        ...
                        ListView.builder(
                          ...
                          itemBuilder: (BuildContext context, int index) {
                            ...
                            return MouseRegion(
                              onEnter:
                                  (_) => setState(
                                    () => _hoveredCircuitIndex = index,
                                  ),
                              onExit:
                                  (_) => setState(
                                    () => _hoveredCircuitIndex = null,
                                  ),
  ...
  Widget _buildCircuitCard({
    required int index,
    required CircuitModel circuitData,
    required List<Speaker> speakersList,
  }) {

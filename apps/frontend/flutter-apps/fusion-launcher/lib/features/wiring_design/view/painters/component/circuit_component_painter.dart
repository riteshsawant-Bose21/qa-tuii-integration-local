...
class CircuitComponentPainter extends ComponentDataPainter {
  final CircuitComponent component;
  final ComponentPainter painter;

  CircuitComponentPainter({required this.component, required this.painter});
  @override
  void paint(Canvas canvas, Size size) {
    final Rect rect = component.position & component.size;

    final CircuitComponentData data = (component.data as CircuitComponentData);
    final ComponentData? speaker = data.speakers.firstOrNull;
    ...
    painter.drawText(
      canvas: canvas,
      text: "${speaker?.label} x ${data.speakers.length}",

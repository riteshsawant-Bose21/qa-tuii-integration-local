...
class _WiringPageState extends State<WiringPage> {
  ...
  void _correctPositionOfNewHardware() {
    ...
    final List<HardwareComponent> allHardware =
        projectViewModel.hardwareComponents.where((HardwareComponent e) => e is! Speaker && e is! HardwareRack).toList();
    final List<Zone> allZones = projectViewModel.zones;

    final List<HardwareComponent> newHardwares = allHardware.where((HardwareComponent e) => e.wiringPos == null).toList();
    final List<Zone> newZones = allZones.where((Zone e) => e.wiringPos == null).toList();
    if (newHardwares.isEmpty && newZones.isEmpty) return;

    final List<WiringLayoutResult> layouts = <WiringLayoutResult>[];

    final WiringZoneManager zoneManager = WiringZoneManager()..syncWithProjectManager(projectViewModel);
    final WiringAutoLayoutUseCase wiringAutoLayoutUseCase = WiringAutoLayoutUseCase(
    ...
    if (newHardwares.length == allHardware.length && newZones.length == allZones.length) {
      layouts.addAll(
        wiringAutoLayoutUseCase.execute(),
      );
      ...
      final FusionCanvasPainter? painter = fusionCanvasPainter;
      if (painter == null) {
        return;
      }
      for (final WiringLayoutResult layout in layouts) {
        ...
        if (layer != null) {
          moveLayer.call(layer, (layout.position - layer.getBounds(painter).topLeft));
        }
      }
    } else {
      layouts.addAll(
        wiringAutoLayoutUseCase.placeTheseDevices(
          newDevices: newHardwares,
          newZones: newZones,
        ),
      );
      final FusionCanvasPainter? painter = fusionCanvasPainter;
      ...
      for (final WiringLayoutResult layout in layouts) {
        final FusionBasePainter? layer = painter.getLayerById(layout.hardwareComponent?.id ?? layout.zone?.id);
        if (layer != null) {
          moveLayer.call(layer, (layout.position - layer.getBounds(painter).topCenter));
        }
      }
    }
  }

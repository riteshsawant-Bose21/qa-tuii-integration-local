...
class AmplifierMatchingWidgetClean extends StatefulWidget {
  ...
  @override
  State<AmplifierMatchingWidgetClean> createState() =>
      _AmplifierMatchingWidgetCleanState();
}

/// Input model for circuit configuration
class CircuitInput {
  String? selectedModel;
  final TextEditingController speakerCountController = TextEditingController(
    text: '1',
  );
  final TextEditingController tapWattsController = TextEditingController(
    text: '15.0',
  );
  final TextEditingController offsetDbController = TextEditingController(
    text: '0.0',
  );
  String mode = 'hi-z';
...
class _AmplifierMatchingWidgetCleanState
    extends State<AmplifierMatchingWidgetClean> {
  ...
  final double _globalVoltage =
      100.0; // Global voltage setting for all Hi-Z circuits

  List<String> get availableSpeakerModels =>
      SpeakerCatalog.database.keys.toList();
  ...
  void _addHighPowerTestCircuit() {
    setState(() {
      ...
      if (highPowerModel != null) {
        ...
        testCircuit.speakerCountController.text =
            '10'; // Many speakers to exceed limits
        testCircuit.tapWattsController.text =
            SpeakerCatalog.database[highPowerModel]?.ppk.toString() ?? '1000';
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text(
          'Added high-power test circuit - this should trigger capacity warnings!',
        ),
  ...
  void _addNormalTestCircuit() {
    setState(() {
      ...
      if (normalPowerModel != null) {
        ...
        testCircuit.tapWattsController.text =
            SpeakerCatalog.database[normalPowerModel]?.ppk.toString() ?? '100';
        testCircuit.offsetDbController.text = '0.0';
      }
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text(
          'Added normal test circuit - this should pass capacity validation',
        ),
  ...
  void _importCircuitingData() {
    if (!_circuitDataService.hasCircuitingData) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'No circuiting data available. Please run circuiting algorithm first.',
          ),
    ...
    setState(() {
      ...
      final List<Circuit> importedCircuits = _circuitDataService
          .convertToCircuits(SpeakerCatalog.database);
      ...
      for (final Circuit circuit in importedCircuits) {
        ...
        circuitInput.speakerCountController.text =
            circuit.speakerCount.toString();
        circuitInput.tapWattsController.text = circuit.tapWatts.toString();
        circuitInput.offsetDbController.text =
            circuit.outputOffsetDb.toString();
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          'Imported ${circuits.length} circuits from circuiting algorithm',
        ),
        backgroundColor: Colors.green,
      ),
    );
        ...
        children: <Widget>[
          ...
          Column(
            ...
            children: <Widget>[
              ...
              ...circuits.asMap().entries.map((
                MapEntry<int, CircuitInput> entry,
              ) {
            ...
            child:
                isLoading
                    ...
                    : ElevatedButton(
                      ...
                      child: const Text(
                        'Calculate Amplifier Matching (Both Strategies)',
                      ),
                    ),
          ),
          ...
          if (symmetricalResult != null && asymmetricalResult != null)
            _buildCapacityValidationSection(
              symmetricalResult!,
              asymmetricalResult!,
            ),
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            FusionDropdownButtonFormField(
              semanticKey: 'circuit_${circuit.circuitId}_speaker_model',
              value: circuit.selectedModel,
              options: availableSpeakerModels,
              // items:
                ...
                setState(() {
                  ...
                  if (value != null) {
                    final SpeakerModel? speaker =
                        SpeakerCatalog.database[value];
                    if (speaker != null) {
                      if (circuit.mode == 'lo-z') {
                        circuit.tapWattsController.text =
                            speaker.ppk.toString();
                      } else if (circuit.mode == 'hi-z' &&
                          speaker.hiZTaps.isNotEmpty) {
                        circuit.selectedTap = speaker.hiZTaps.first;
                        circuit.tapWattsController.text =
                            speaker.hiZTaps.first.toString();
                      }
                    }
                  }
                });
              },
              hintText: 'Speaker Model',
                    ...
                    onChanged: (String? value) {
                      setState(() {
                        ...
                        if (circuit.selectedModel != null) {
                          final SpeakerModel? speaker =
                              SpeakerCatalog.database[circuit.selectedModel!];
                          if (speaker != null) {
                            if (circuit.mode == 'lo-z') {
                              circuit.tapWattsController.text =
                                  speaker.ppk.toString();
              ...
              children: <Widget>[
                Expanded(
                  child: FusionTextFormField(
                    semanticId: 'circuit_${circuit.circuitId}_speaker_count',
                    title: "Speaker Count",
                    hintText: "",
                ...
                if (circuit.mode == 'hi-z')
                  ...
                  Expanded(
                    child: FusionTextFormField(
                      semanticId: 'circuit_${circuit.circuitId}_peak_watts',
                      controller: circuit.tapWattsController,
                      title: "Peak Power (P_pk)",
                      hintText: "",
                      // decoration: const InputDecoration(
                      //   labelText: 'Peak Power (P_pk)',
                      //   border: OutlineInputBorder(),
                      //   suffixIcon: Icon(Icons.auto_awesome, size: 16),
                      // ),
                      keyboardType: TextInputType.number,
                      readOnly: true,
                    ),
                ...
                Expanded(
                  child: FusionTextFormField(
                    semanticId: 'circuit_${circuit.circuitId}_offset_db',
  ...
  Widget _buildCapacityValidationSection(
    AmpMatchingResult symmetrical,
    AmpMatchingResult asymmetrical,
  ) {
    final bool hasErrors = symmetrical.hasErrors || asymmetrical.hasErrors;
    final bool hasWarnings =
        symmetrical.hasWarnings || asymmetrical.hasWarnings;
                  ...
                  Container(
                    ...
                    child: const Column(
                      ...
                      children: <Widget>[
                        ...
                        SizedBox(height: 4),
                        Text(
                          '• Reduce circuit power requirements (fewer speakers or lower power settings)',
                        ),
                        Text('• Use higher-capacity amplifiers if available'),
                        Text(
                          '• Consider splitting circuits across multiple amplifiers',
                        ),
                        Text(
                          '• Check if any circuits exceed individual amplifier limits',
                        ),
                      ],
                    ),
                  ),
  ...
  Widget _buildSideBySideResults(
    AmpMatchingResult symmetrical,
    AmpMatchingResult asymmetrical,
  ) {
    return Column(
      ...
      children: <Widget>[
        ...
        Card(
          ...
          child: Padding(
            ...
            child: Row(
              children: <Widget>[
                Icon(Icons.music_note, color: context.colorScheme.white),
                const SizedBox(width: 8),
                Text(
  ...
  Widget _buildDetailedTechnicalAnalysis(
    String title,
    AmpMatchingResult result,
    Color color,
    PowerAllocationStrategy strategy,
    IconData icon,
  ) {
    return Card(
      elevation: 3,
          ...
          for (int i = 0; i < circuits.length; i++) ...<Widget>[
            Container(
              ...
              child: Column(
                ...
                children: <Widget>[
                  Text(
                    'Circuit ${i + 1}: ${circuits[i].selectedModel ?? "Not selected"}',
                    style: const TextStyle(fontWeight: FontWeight.w500),
                  ),
                  Text('  • Type: ${circuits[i].mode.toUpperCase()}'),
                  Text(
                    '  • Count: ${circuits[i].speakerCountController.text} speakers',
                  ),
                  if (circuits[i].mode == 'hi-z')
                    Text(
                      '  • Tap Power: ${circuits[i].tapWattsController.text}W per speaker',
                    ),
                  if (double.tryParse(circuits[i].offsetDbController.text) != 0)
                    Text(
                      '  • Offset: ${circuits[i].offsetDbController.text}dB',
                    ),
                  Text(
                    '  • Estimated Power: ${_estimateCircuitPower(circuits[i]).toStringAsFixed(1)}W',
                  ),
                ],
              ),
            ),
      ...
      children: <Widget>[
        ...
        Container(
          ...
          child: const Column(
            ...
            children: <Widget>[
              Text(
                'Hi-Z Formula: P_base = N_speakers × W_tap × 2',
                style: TextStyle(
              ...
              Text(
                'Lo-Z Formula: P_base = N_speakers × P_peak(speaker_model)',
                style: TextStyle(
                  fontFamily: 'monospace',
                  fontWeight: FontWeight.bold,
                ),
              ...
              Text(
                'Offset Formula: P_actual = P_base × 10^(-dB/10)',
                style: TextStyle(
                  fontFamily: 'monospace',
                  fontWeight: FontWeight.bold,
                ),
        ...
        if (circuits.isNotEmpty) ...<Widget>[
          ...
          Container(
            ...
            child: const Text(
              'Add circuits to see actual calculations',
              style: TextStyle(fontStyle: FontStyle.italic),
            ),
          ),
        ],
  ...
  Widget _buildActualPowerCalculation(CircuitInput circuit) {
    ...
    final int speakerCount =
        int.tryParse(circuit.speakerCountController.text) ?? 1;
    final double tapWatts =
        double.tryParse(circuit.tapWattsController.text) ?? 0.0;
    final double offsetDb =
        double.tryParse(circuit.offsetDbController.text) ?? 0.0;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          Text(
            'Circuit ${circuit.circuitId}: ${circuit.selectedModel ?? "No model selected"}',
          ...
          if (circuit.selectedModel != null) ...<Widget>[
            const SizedBox(height: 4),
            Text(
              '1. Base Power: $powerSource = ${originalPower.toStringAsFixed(1)}W',
            ),

            if (offsetDb != 0) ...<Widget>[
              Text('2. Offset: ${offsetDb}dB'),
              Text(
                '3. Multiplier: 10^(-$offsetDb/10) = ${multiplier.toStringAsFixed(3)}',
              ),
              Text(
                '4. Final Power: ${originalPower.toStringAsFixed(1)}W × ${multiplier.toStringAsFixed(3)} = ${finalPower.toStringAsFixed(1)}W',
                style: const TextStyle(
                  fontWeight: FontWeight.bold,
                  color: Colors.orange,
                ),
              ...
              Text(
                'Final Power: ${originalPower.toStringAsFixed(1)}W',
                style: const TextStyle(
                  fontWeight: FontWeight.bold,
                  color: Colors.green,
                ),
            ...
            const Text(
              '⚠️ Select a speaker model to see calculations',
  ...
  Widget _buildCircuitSortingStep() {
    ...
    sortedCircuits.sort(
      (CircuitInput a, CircuitInput b) =>
          _estimateCircuitPower(b).compareTo(_estimateCircuitPower(a)),
    );
    ...
    return Column(
      ...
      children: <Widget>[
        const Text(
          '📊 Circuits sorted by power (highest to lowest):',
          style: TextStyle(fontWeight: FontWeight.bold),
        ),
        const SizedBox(height: 8),
                ...
                children: <Widget>[
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: <Widget>[
                        Text(
                          '${sortedCircuits[i].selectedModel ?? "Unknown"} (${sortedCircuits[i].mode.toUpperCase()})',
                          style: const TextStyle(fontWeight: FontWeight.w500),
                        ),
                        Text(
                          '${_estimateCircuitPower(sortedCircuits[i]).toStringAsFixed(1)}W - ${sortedCircuits[i].speakerCountController.text} speakers',
                        ),
                      ],
                    ),
                  ),
  ...
  Widget _buildAmplifierTierMatchingStep(PowerAllocationStrategy strategy) {
    return Column(
      ...
      children: <Widget>[
        const Text(
          '🎯 Amplifier Tier Matching Algorithm:',
          style: TextStyle(fontWeight: FontWeight.bold),
        ),
        const SizedBox(height: 8),
        ...
        Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              const Text(
                'Tier Rule: Ppk_amplifier (next tier up) ≥ Pk_speaker_total ≥ Ppk_amplifier (current tier)',
                style: TextStyle(
                  fontFamily: 'monospace',
                  fontWeight: FontWeight.bold,
                  fontSize: 16,
              ...
              const Text(
                '• Continue pattern: unused channels on smallest amplifiers',
              ),
              const SizedBox(height: 8),
              const Text(
                'Available Amplifier Tiers:',
                style: TextStyle(fontWeight: FontWeight.bold),
              ),
              ...AmplifierCatalog.allModels.map(
                (AmplifierModel amp) => Text(
                  '• ${amp.name}: ${amp.symmetrical.watts}W/ch, ${amp.symmetrical.totalCapacity}W total, ${amp.channels} channels',
                ),
              ),
            ],
          ),
        ...
        if (circuits.isNotEmpty) ...<Widget>[
          ...
          Container(
            ...
            child: const Text(
              'Add circuits to see amplifier matching process',
              style: TextStyle(fontStyle: FontStyle.italic),
            ),
          ),
        ],
  ...
  List<Map<String, dynamic>> _calculateOptimalAmplifierAllocation(
    ...
    final List<Map<String, dynamic>> amplifierAllocation =
        <Map<String, dynamic>>[];
    ...
    final List<Map<String, dynamic>> sortedCircuits =
        List<Map<String, dynamic>>.from(circuitData);
    sortedCircuits.sort(
    ...
    for (final Map<String, dynamic> circuit in sortedCircuits) {
      ...
      for (final Map<String, dynamic> amp in amplifierAllocation) {
        ...
        final List<Map<String, dynamic>> assignedCircuits =
            amp['circuitsAssigned'] as List<Map<String, dynamic>>;
        final double currentTotalPower = assignedCircuits.fold(
          0.0,
          (double sum, Map<String, dynamic> c) => sum + (c['power'] as double),
        );
        final double newTotalPower =
            currentTotalPower + (circuit['power'] as double);
  ...
  Widget _buildAsymmetricalInitialAnalysis() {
    ...
    final double maxCircuitPower =
        circuitData.isNotEmpty ? circuitData.first['power'] as double : 0.0;
    final List<AmplifierModel> amplifiers = AmplifierCatalog.allModels;
    ...
    if (selectedTier == null) {
      return Column(
        children: <Widget>[
          const Text(
            '❌ Error: No amplifier can handle the maximum circuit power',
          ),
          Text('Maximum circuit power: ${maxCircuitPower.toStringAsFixed(1)}W'),
          Text(
            'Maximum amplifier capacity: ${amplifiers.last.asymmetrical.watts}W',
          ),
        ],
      );
    }
      ...
      children: <Widget>[
        ...
        Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              Text(
                'Total System Power: ${totalSystemPower.toStringAsFixed(1)}W',
              ),
              Text(
                'Maximum Circuit Power: ${maxCircuitPower.toStringAsFixed(1)}W',
              ),
              Text('Selected Amplifier: ${selectedTier.name}'),
              Text(
                'Asymmetrical Per-Channel Capacity: ${selectedTier.asymmetrical.watts}W',
              ),
              Text(
                'Total Asymmetrical Capacity: ${selectedTier.asymmetrical.totalCapacity}W',
              ),
            ],
          ),
        ),
        ...
        Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              const Text(
                '📋 Circuits (Sorted by Power):',
                style: TextStyle(fontWeight: FontWeight.bold),
              ),
              const SizedBox(height: 8),
              ...circuitData.asMap().entries.map((
                ...
                return Padding(
                  ...
                  child: Text(
                    '${index + 1}. ${circuit['model']} (${circuit['mode']}) - ${(circuit['power'] as double).toStringAsFixed(1)}W - ${circuit['speakerCount']} speakers',
                  ),
                );
              }),
            ],
  ...
  Widget _buildCircuitMovementOptimization() {
      ...
      'amplifiers': <Map<String, dynamic>>[
        <String, dynamic>{
          ...
          'channels': List.generate(
            selectedTier.channels,
            (int index) => <String, dynamic>{
              'channelNumber': index + 1,
              'circuit': index < circuitData.length ? circuitData[index] : null,
              'power':
                  index < circuitData.length
                      ? circuitData[index]['power'] as double
                      : 0.0,
            },
          ),
          'netPowerSharing': _calculateNetPowerSharing(
            circuitData,
            selectedTier,
          ),
          'availableToShare': _calculateAvailableToShare(
            circuitData,
            selectedTier,
          ),
        },
    ...
    final double netPowerSharing = _calculateNetPowerSharing(
      circuitData,
      selectedTier,
    );
    final double availableToShare = _calculateAvailableToShare(
      circuitData,
      selectedTier,
    );
          ...
          <String, dynamic>{
            ...
            'channels': List.generate(
              ...
              (int index) => <String, dynamic>{
                'channelNumber': index + 1,
                'circuit':
                    index < circuitData.length ? circuitData[index] : null,
                'power':
                    index < circuitData.length
                        ? circuitData[index]['power'] as double
                        : 0.0,
                'needsExtraPower':
                    index < circuitData.length &&
                    (circuitData[index]['power'] as double) >
                        selectedTier!.symmetrical.watts,
              },
            ),
    ...
    return Column(
      children: <Widget>[
        const Text(
          '🔄 Circuit Movement Optimization Process',
          style: TextStyle(fontWeight: FontWeight.bold),
        ),
        const SizedBox(height: 8),
        const Text(
          'This step shows the step-by-step process of optimizing circuit assignments for maximum power sharing efficiency.',
        ),
        const SizedBox(height: 16),

        // Optimization Steps
        ...optimizationSteps.map(
          (Map<String, dynamic> step) => _buildOptimizationStep(step),
        ),
        ...
        Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              Text(
                'Available to Share: ${availableToShare.toStringAsFixed(1)}W',
              ),
              Text(
                'Optimization Status: ${netPowerSharing > 0 ? 'Applied' : 'No optimization needed'}',
              ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          // Amplifier details
          ...(step['amplifiers'] as List<Map<String, dynamic>>).map(
            (Map<String, dynamic> amp) => Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: <Widget>[
                Text(
                  'Amplifier: ${amp['name']}',
                ...
                // Channel details
                ...(amp['channels'] as List<Map<String, dynamic>>).map(
                  (Map<String, dynamic> channel) => Padding(
                    padding: const EdgeInsets.only(left: 16, top: 2),
                    child: Text(
                      'Ch ${channel['channelNumber']}: ${channel['circuit'] != null ? '${channel['circuit']['model']} (${(channel['power'] as double).toStringAsFixed(1)}W)' : 'N/A'}',
                    ),
                  ),
                ),
                ...
                const SizedBox(height: 4),
                Text(
                  'Net Power Sharing: ${(amp['netPowerSharing'] as double).toStringAsFixed(1)}W',
                ),
                Text(
                  'Available to Share: ${(amp['availableToShare'] as double).toStringAsFixed(1)}W',
                ),

                if (amp['canOptimize'] == true)
                  const Text(
                    '✅ Can optimize power sharing',
  ...
  double _calculateNetPowerSharing(
    List<Map<String, dynamic>> circuitData,
    ...
    for (int i = 0; i < amplifier.channels; i++) {
      final double circuitPower =
          i < circuitData.length ? circuitData[i]['power'] as double : 0.0;
      final double symmSurplus = math.max(
        perChannelCapacity - circuitPower,
        0.0,
      );
      final double asymmDeficit = math.min(
        circuitPower - perChannelCapacity,
        0.0,
      );
  ...
  double _calculateAvailableToShare(
    List<Map<String, dynamic>> circuitData,
    AmplifierModel amplifier,
  ) {
    final double perChannelCapacity = amplifier.asymmetrical.watts.toDouble();
  ...
  Widget _buildFinalTierRuleValidation() {
    ...
    // Check current tier
    final bool currentTierValid =
        totalSystemPower >= selectedTier.symmetrical.totalCapacity;
    ...
    final bool nextTierUpValid =
        nextTierUp == null ||
        totalSystemPower <= nextTierUp.symmetrical.totalCapacity;
    ...
    return Column(
      children: <Widget>[
        const Text(
          '✅ Final Tier Rule Validation',
          style: TextStyle(fontWeight: FontWeight.bold),
        ),
        const SizedBox(height: 8),
        const Text(
          'Validating tier rule: Ppk_amplifier (next tier up) ≥ Pk_speaker_total ≥ Ppk_amplifier (current tier)',
        ),
        const SizedBox(height: 16),

        // Validation Results
        ...validationResults.map(
          (Map<String, dynamic> result) => _buildValidationResult(result),
        ),
          ...
          Container(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                const SizedBox(height: 8),
                Text(
                  'Current tier ${selectedTier.name} cannot handle total system power.',
                ),
                Text(
                  'Recommended tier: ${recommendedTier?.name ?? 'None available'}',
                ),
                Text(
                  'Reason: Total system power (${totalSystemPower.toStringAsFixed(1)}W) > Current tier capacity (${selectedTier.symmetrical.totalCapacity}W)',
                ),
              ],
            ),
          ),
        ...
        Container(
          ...
          decoration: BoxDecoration(
            color:
                (currentTierValid && nextTierUpValid)
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              Text(
                'Tier Rule Satisfied: ${(currentTierValid && nextTierUpValid) ? 'YES' : 'NO'}',
              ),
              Text('Current Tier Valid: ${currentTierValid ? 'YES' : 'NO'}'),
              Text('Next Tier Up Valid: ${nextTierUpValid ? 'YES' : 'NO'}'),
              Text(
                'Final Amplifier: ${needsDowngrade ? (recommendedTier?.name ?? 'None') : selectedTier.name}',
              ),
            ],
          ),
        ),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: <Widget>[
          ...
          Text(
            'Total System Power: ${(result['totalSystemPower'] as double).toStringAsFixed(1)}W',
          ),
          const SizedBox(height: 8),
          Text(
            'Current Tier Valid: ${result['currentTierValid'] ? '✅ YES' : '❌ NO'}',
          ),
          Text(
            'Next Tier Up Valid: ${result['nextTierUpValid'] ? '✅ YES' : '❌ NO'}',
          ),
  ...
  Widget _buildChannelBasedResults() {
    ...
    final List<Map<String, dynamic>> amplifierAllocation =
        _calculateOptimalAmplifierAllocation(
          circuitData,
    ...
    final List<Map<String, dynamic>> optimizedAllocation =
        _applyCircuitMovementOptimization(amplifierAllocation, circuitData);
    ...
    for (int ampIndex = 0; ampIndex < optimizedAllocation.length; ampIndex++) {
      ...
      final List<Map<String, dynamic>> assignedCircuits =
          amplifier['circuitsAssigned'] as List<Map<String, dynamic>>;

      // Calculate power sharing for this specific amplifier using AmplifierCatalog data
      final Widget ampAnalysis = _buildAmplifierPowerSharingAnalysis(
        ampModel,
        assignedCircuits,
        ampIndex + 1,
      );
    ...
    return Column(
      children: <Widget>[
        const Text(
          '📊 Mixed Amplifier Power Sharing Analysis',
          style: TextStyle(fontWeight: FontWeight.bold),
        ),
        const SizedBox(height: 8),
        const Text(
          'Complete power sharing analysis for each amplifier with circuit movement optimization',
        ),
        const SizedBox(height: 16),
        ...
        Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              const Text(
                '🔧 Mixed Amplifier Allocation (Cost Optimized):',
                style: TextStyle(fontWeight: FontWeight.bold),
              ),
              const SizedBox(height: 8),
              Text('Total Amplifiers Required: ${optimizedAllocation.length}'),
              const SizedBox(height: 8),
              ...optimizedAllocation.map(
                (Map<String, dynamic> amp) => Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: <Widget>[
                    Text(
                      '• ${amp['tier']} (${amp['usedChannels']}/${amp['totalChannels']} channels used)',
                    ),
                    Text(
                      '  Circuits: ${(amp['circuitsAssigned'] as List).map((c) => '${c['model']} (${c['power'].toStringAsFixed(1)}W)').join(', ')}',
                    ),
                    const SizedBox(height: 4),
                  ],
                ),
              ...
              const SizedBox(height: 8),
              const Text(
                'Strategy: Fill existing amplifiers first, then add new ones for cost optimization',
              ),
              const SizedBox(height: 8),
              const Text(
                'Circuit Movement Optimization Applied:',
                style: TextStyle(fontWeight: FontWeight.bold),
              ),
              const Text(
                '• Moved circuits from higher-power amplifiers to amplifiers with available power sharing',
              ),
              const Text(
                '• Optimized until no amplifiers have sufficient "Net power sharing" for additional circuits',
              ),
            ],
          ),
        ),
  ...
  /// Apply circuit movement optimization logic
  List<Map<String, dynamic>> _applyCircuitMovementOptimization(
    List<Map<String, dynamic>> amplifierAllocation,
    List<Map<String, dynamic>> circuitData,
  ) {
    ...
    for (final Map<String, dynamic> amp in amplifierAllocation) {
      optimizedAllocation.add(<String, dynamic>{
        ...
        'circuitsAssigned': List<Map<String, dynamic>>.from(
          amp['circuitsAssigned'] as List,
        ),
      });
    }
    ...
    while (movedCircuit && iterations < maxIterations) {
      ...
      for (final Map<String, dynamic> amp in optimizedAllocation) {
        ...
        for (int i = 0; i < ampModel.channels; i++) {
          final double actualPowerNeeded =
              i < assignedCircuits.length
                  ? assignedCircuits[i]['power'] as double
                  : 0.0;
          final double symmSurplus = math.max(
            perChannelCapacity - actualPowerNeeded,
            0.0,
          );
          final double asymmDeficit = math.min(
            perChannelCapacity - actualPowerNeeded,
      ...
      for (final Map<String, dynamic> sourceAmp in optimizedAllocation) {
        final double sourceNetPowerSharing =
            sourceAmp['netPowerSharing'] as double;
        ...
        if (sourceNetPowerSharing > 0 &&
            sourceUsedChannels < sourceTotalChannels) {
          ...
          for (final Map<String, dynamic> targetAmp in optimizedAllocation) {
            ...
            final List<Map<String, dynamic>> targetCircuits =
                targetAmp['circuitsAssigned'] as List<Map<String, dynamic>>;
            if (targetCircuits.isEmpty) continue;

            // Find the highest power circuit on the target amplifier
            final Map<String, dynamic> highestCircuit = targetCircuits.reduce(
              (Map<String, dynamic> a, Map<String, dynamic> b) =>
            ...
            final AmplifierModel sourceAmpModel =
                sourceAmp['tierModel'] as AmplifierModel;
            final List<Map<String, dynamic>> sourceCircuits =
                sourceAmp['circuitsAssigned'] as List<Map<String, dynamic>>;

            // Calculate current total power on source amplifier
            final double currentSourcePower = sourceCircuits.fold(
              0.0,
              (double sum, Map<String, dynamic> circuit) =>
                  sum + (circuit['power'] as double),
            );
            ...
            if (circuitPower <= sourceNetPowerSharing &&
              ...
              (sourceAmp['circuitsAssigned'] as List<Map<String, dynamic>>).add(
                highestCircuit,
              );

              // Update channel counts
              sourceAmp['usedChannels'] =
                  (sourceAmp['usedChannels'] as int) + 1;
              sourceAmp['unusedChannels'] =
                  (sourceAmp['unusedChannels'] as int) - 1;
              targetAmp['usedChannels'] =
                  (targetAmp['usedChannels'] as int) - 1;
              targetAmp['unusedChannels'] =
                  (targetAmp['unusedChannels'] as int) + 1;
  ...
  Widget _buildAmplifierPowerSharingAnalysis(
    AmplifierModel amplifier,
    List<Map<String, dynamic>> assignedCircuits,
    int amplifierNumber,
  ) {
    // Use AmplifierCatalog data properly
    final int channelCount = amplifier.channels;
    final double perChannelCapacity =
        amplifier.symmetrical.watts.toDouble(); // L = per-channel limit
    final double totalCapacity =
        amplifier.asymmetrical.totalCapacity.toDouble();
    final double asymmetricalShareablePerChannel =
        amplifier.asymmetrical.watts.toDouble();
    ...
    final double sharePerNeeding =
        channelsNeedingPower > 0
            ? availableToShare / channelsNeedingPower
            : 0.0;
    ...
    for (int i = 0; i < channelCount; i++) {
      ...
      final double powerAvailable =
          needsExtraPower
              ? perChannelCapacity + sharePerNeeding
              : perChannelCapacity;
      final double limitPowerDelivered = math.min(
        powerAvailable,
        asymmetricalShareablePerChannel,
      );
      final double powerDelivered = math.min(
        limitPowerDelivered,
        actualPowerNeeded,
      );

      // STEP 8: Headroom Calculations formulas (from sample.md)
      final double channelHeadroom =
          powerDelivered > 0
              ? 10 *
                  math.log(asymmetricalShareablePerChannel / powerDelivered) /
                  math.ln10
              : 0.0;
      final double loudspeakerHeadroom =
          powerDelivered > 0
              ? 10 * math.log(actualPowerNeeded / powerDelivered) / math.ln10
              : 0.0;

      // Update existing channel data with remaining calculations
      channel['netPowerSharing'] =
          netPowerSharing; // Global value for all channels
      channel['availableToShare'] =
          availableToShare; // Global value for all channels
      channel['sharePerNeeding'] =
          sharePerNeeding; // Global value for all channels
    ...
    final double totalActualPower = channelData.fold(
      0.0,
      (double sum, Map<String, dynamic> ch) =>
          sum + (ch['actualPowerNeeded'] as double),
    );
    final double totalPowerDelivered = channelData.fold(
      0.0,
      (double sum, Map<String, dynamic> ch) =>
          sum + (ch['powerDelivered'] as double),
    );
    final double totalAmpHeadroom =
        totalPowerDelivered > 0
            ? 10 * math.log(totalCapacity / totalPowerDelivered) / math.ln10
            : 0.0;
    ...
    for (final Map<String, dynamic> channel in channelData) {
      ...
      channel['effectiveHeadroom'] = math.min(
        channelHeadroom,
        totalAmpHeadroom,
      );
    }
        ...
        children: <Widget>[
          ...
          Container(
            ...
            child: Column(
              ...
              children: <Widget>[
                Text(
                  '🔧 Amplifier $amplifierNumber: ${amplifier.name}',
                  style: const TextStyle(
                    fontWeight: FontWeight.bold,
                    fontSize: 16,
                  ),
                ...
                const SizedBox(height: 8),
                Text(
                  'Per-Channel Limit (L): ${perChannelCapacity.toStringAsFixed(1)}W',
                ),
                Text(
                  'Asymmetrical Shareable Per Channel: ${asymmetricalShareablePerChannel.toStringAsFixed(1)}W',
                ),
                Text('Total Capacity: ${totalCapacity.toStringAsFixed(1)}W'),
                const SizedBox(height: 8),
                Text(
                  'Total Symm Surplus: ${totalSymmSurplus.toStringAsFixed(1)}W',
                ),
                Text(
                  'Total Asymm Deficit: ${totalAsymmDeficit.toStringAsFixed(1)}W',
                ),
                Text(
                  'Net Power Sharing: ${netPowerSharing.toStringAsFixed(1)}W',
                ),
                Text(
                  'Available to Share: ${availableToShare.toStringAsFixed(1)}W',
                ),
                ...
                Text(
                  'Share Per Needing: ${sharePerNeeding.toStringAsFixed(1)}W',
                ),
                Text(
                  'Total Amp Headroom: ${totalAmpHeadroom.toStringAsFixed(1)} dB',
                ),
              ],
            ),
          ),
          ...
          Container(
            ...
            child: Column(
              ...
              children: <Widget>[
                Text(
                  '🔍 Amplifier $amplifierNumber Validation:',
                  style: TextStyle(
                    fontWeight: FontWeight.bold,
                    color: systemHasEnoughPower ? Colors.green : Colors.red,
                  ),
                ),
                const SizedBox(height: 8),
                Text(
                  'Power Conservation: ${totalPowerDelivered.toStringAsFixed(1)}W ≤ ${totalCapacity.toStringAsFixed(1)}W ${totalPowerDelivered <= totalCapacity ? '✓' : '❌'}',
                ),
                Text(
                  'Power Sufficiency: ${systemHasEnoughPower ? 'All circuits receive sufficient power ✓' : 'Some circuits insufficient power ❌'}',
                ),
                Text(
                  'Net Power Sharing: ${netPowerSharing.toStringAsFixed(1)}W ≥ 0 ${netPowerSharing >= 0 ? '✓' : '❌'}',
                ),
                const SizedBox(height: 8),
                Text(
                  'Result: ${systemHasEnoughPower ? '✅ VALIDATION PASSED' : '❌ VALIDATION FAILED'}',
                ),
              ],
            ),
          ),
  ...
  Widget _buildCompleteChannelTable(
    ...
    return Column(
      children: <Widget>[
        ...
        _buildTableRow(
          ...
          channelData
              .map(
                (Map<String, dynamic> ch) => ch['actualPowerNeeded'] as double,
              )
              .toList(),
        ),
        _buildTableRow(
          'Symm Surplus',
          channelData
              .map((Map<String, dynamic> ch) => ch['symmSurplus'] as double)
              .toList(),
        ),
        _buildTableRow(
          'Asymm Deficit',
          channelData
              .map((Map<String, dynamic> ch) => ch['asymmDeficit'] as double)
              .toList(),
        ),
        _buildTableRow(
          'Needs Extra Power',
          channelData
              .map(
                (Map<String, dynamic> ch) =>
                    (ch['needsExtraPower'] as bool) ? 'YES' : '',
              )
              .toList(),
        ),
        _buildTableRow(
          'Power Available',
          channelData
              .map((Map<String, dynamic> ch) => ch['powerAvailable'] as double)
              .toList(),
        ),
        _buildTableRow(
          'Limit Power Delivered',
          channelData
              .map(
                (Map<String, dynamic> ch) =>
                    ch['limitPowerDelivered'] as double,
              )
              .toList(),
        ),
        _buildTableRow(
          'Power Delivered',
          channelData
              .map((Map<String, dynamic> ch) => ch['powerDelivered'] as double)
              .toList(),
        ),
        _buildTableRow(
          'Channel Headroom (dB)',
          channelData
              .map((Map<String, dynamic> ch) => ch['channelHeadroom'] as double)
              .toList(),
        ),
        _buildTableRow(
          'Effective Headroom (dB)',
          channelData
              .map(
                (Map<String, dynamic> ch) => ch['effectiveHeadroom'] as double,
              )
              .toList(),
        ),
        _buildTableRow(
          'Loudspeaker Headroom (dB)',
          channelData
              .map(
                (Map<String, dynamic> ch) =>
                    ch['loudspeakerHeadroom'] as double,
              )
              .toList(),
        ),
      ],
  ...
  Widget _buildChannelBasedTable(
    List<Map<String, dynamic>> circuitData,
    AmplifierModel amplifier,
  ) {
    final int channelCount = amplifier.channels;
    ...
    final double totalSymmSurplus = channelData.fold(
      0.0,
      (double sum, Map<String, dynamic> ch) =>
          sum + (ch['symmSurplus'] as double),
    );
    final double totalAsymmDeficit = channelData.fold(
      0.0,
      (double sum, Map<String, dynamic> ch) =>
          sum + (ch['asymmDeficit'] as double),
    );
    final double netPowerSharing = totalSymmSurplus + totalAsymmDeficit;
        ...
        Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              Text(
                'Total System Power: ${totalActualPower.toStringAsFixed(1)}W',
              ),
              Text(
                'Amplifier Capacity: ${amplifier.asymmetrical.totalCapacity}W',
              ),
              Text(
                'System Validation: ${totalActualPower <= amplifier.asymmetrical.totalCapacity ? 'YES' : 'NO'}',
              ),
            ],
          ),
        ),
  ...
  Widget _buildAmplifierMatchingProcess(PowerAllocationStrategy strategy) {
    ...
    if (selectedTier == null) {
      return Container(
        ...
        child: Column(
          ...
          children: <Widget>[
            const Text(
              '❌ SYSTEM EXCEEDS MAXIMUM AMPLIFIER CAPACITY',
              style: TextStyle(fontWeight: FontWeight.bold, color: Colors.red),
            ),
            const SizedBox(height: 4),
            if (maxCircuitPower > largestAmp.symmetrical.watts) ...<Widget>[
              Text(
                'Largest Circuit: ${maxCircuitPower.toStringAsFixed(1)}W > ${largestAmp.symmetrical.watts}W per-channel (${largestAmp.name})',
              ),
              const Text(
                'Recommendation: Reduce individual circuit power or split into multiple circuits',
              ),
            ] else ...<Widget>[
              Text(
                'Total Power: ${totalSystemPower.toStringAsFixed(1)}W > ${AmplifierCatalog.psx4804d.symmetrical.totalCapacity}W (${AmplifierCatalog.psx4804d.name})',
              ),
              const Text(
                'Recommendation: Reduce circuit power requirements or use multiple amplifiers',
              ),
            ],
          ],
        ),
    ...
    final List<Map<String, dynamic>> amplifierAllocation =
        _calculateOptimalAmplifierAllocation(circuitData, selectedTier);
      ...
      children: <Widget>[
        ...
        Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              const Text(
                '🔌 Circuit Details (Sorted by Power):',
                style: TextStyle(fontWeight: FontWeight.bold),
              ),
              const SizedBox(height: 8),
              ...circuitData.asMap().entries.map((
                    ...
                    children: <Widget>[
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: <Widget>[
                            Text(
                              'Circuit ${circuit['id']}: ${circuit['model']} (${circuit['mode'].toUpperCase()})',
                              style: const TextStyle(
                                fontWeight: FontWeight.w500,
                              ),
                            ),
                            Text(
                              '${(circuit['power'] as double).toStringAsFixed(1)}W - ${circuit['speakerCount']} speakers',
                            ),
                          ],
                        ),
                      ),
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              ...amplifierAllocation.map((Map<String, dynamic> amp) {
                final List<Map<String, dynamic>> circuitsAssigned =
                    amp['circuitsAssigned'] as List<Map<String, dynamic>>;
                return Container(
                  margin: const EdgeInsets.only(bottom: 8),
                  padding: const EdgeInsets.all(8),
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                      Text(
                        'Amplifier ${amp['amplifierNumber']}: ${amp['tier']}',
                        style: const TextStyle(fontWeight: FontWeight.bold),
                      ),
                      Text(
                        'Channels: ${amp['usedChannels']}/${amp['totalChannels']} used (${amp['unusedChannels']} unused)',
                      ),
                      if (circuitsAssigned.isNotEmpty) ...<Widget>[
                        ...
                        ...circuitsAssigned.map(
                          (Map<String, dynamic> circuit) => Text(
                            '  • Circuit ${circuit['id']}: ${circuit['model']} (${(circuit['power'] as double).toStringAsFixed(1)}W)',
                          ),
                        ),
                      ],
                    ],
        ...
        Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              const Text(
                'Rule: Ppk_amplifier (next tier up) ≥ Pk_speaker_total ≥ Ppk_amplifier (current tier)',
              ),
              Text(
                'Total System Power: ${totalSystemPower.toStringAsFixed(1)}W',
              ),
              Text(
                'Selected Tier: ${selectedTier.name} (${selectedTier.symmetrical.totalCapacity}W total)',
              ),
              Text(
                'Validation: ${selectedTier.symmetrical.totalCapacity}W ≥ ${totalSystemPower.toStringAsFixed(1)}W ✅',
              ),
              const SizedBox(height: 4),
              const Text(
                'Channel Strategy: Unused channels allocated to smallest amplifiers for cost optimization ✅',
              ),
              const SizedBox(height: 4),
              Text(
                'Amplifier Types Used: ${amplifierAllocation.map((Map<String, dynamic> amp) => amp['tier']).toSet().join(', ')}',
              ),
            ],
          ),
        ),
      ...
      children: <Widget>[
        ...
        Container(
          ...
          child: const Column(
            ...
            children: <Widget>[
              Text(
                'Corrected Asymmetrical Power Sharing Formulas:',
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16),
              ),
              SizedBox(height: 8),
              Text(
                'Symm_Surplus_i = Max[(L - P_actual,i), 0]',
                style: TextStyle(fontFamily: 'monospace'),
              ),
              Text(
                'Asymm_Deficit_i = Min[(L - P_actual,i), 0]',
                style: TextStyle(fontFamily: 'monospace'),
              ),
              Text(
                'NetPower_Sharing = Total_Symm_Surplus + Total_Asymm_Deficit',
                style: TextStyle(fontFamily: 'monospace'),
              ),
            ],
          ),
  ...
  Widget _buildPowerSharingCalculation() {
    ...
    if (circuitData.isEmpty) {
      return Container(
        ...
        child: const Text(
          'Select speaker models to see calculations',
          style: TextStyle(fontStyle: FontStyle.italic),
        ),
      );
    }
    ...
    return Column(
      children: <Widget>[
        ...
        _buildAmplifierAnalysis(
          AmplifierCatalog.psx2404d.name,
        ...
        _buildAmplifierAnalysis(
          AmplifierCatalog.psx4804d.name,
  ...
  Widget _buildAmplifierAnalysis(
    String ampName,
    double perChannelLimit,
    double totalCapacity,
    double asymmetricalShareable,
    List<Map<String, dynamic>> circuitData,
    double totalPower,
  ) {
    ...
    for (int i = 0; i < 4; i++) {
      final double channelPower =
          i < circuitData.length ? circuitData[i]['power'] as double : 0.0;

      // CORRECTED FORMULAS from sample.md
      final double symmSurplus = math.max(
        perChannelLimit - channelPower,
        0.0,
      ); // Positive or zero
      final double asymmDeficit = math.min(
        perChannelLimit - channelPower,
      ...
      channelAnalysis.add(<String, dynamic>{
        ...
        'circuitModel':
            i < circuitData.length ? circuitData[i]['model'] : 'Empty',
      });
    ...
    final int channelsNeedingPower =
        channelAnalysis
            .where((Map<String, dynamic> ch) => ch['needsSharing'] as bool)
            .length;
    final double sharePerNeeding =
        channelsNeedingPower > 0
            ? totalSymmSurplus / channelsNeedingPower
            : 0.0;
    ...
    for (final Map<String, dynamic> channel in channelAnalysis) {
      ...
      final double powerAvailable =
          needsSharing ? perChannelLimit + sharePerNeeding : perChannelLimit;

      // Limit power delivered (asymmetrical max)
      final double limitPowerDelivered = math.min(
        powerAvailable,
        asymmetricalShareable,
      );

      // Power delivered to channel
      final double powerDeliveredToChannel = math.min(
        limitPowerDelivered,
        powerNeeded,
      );
    ...
    // Calculate headroom
    final double totalAmpHeadroom =
        10 * math.log(totalCapacity / totalDelivered) / math.ln10;
          ...
          Container(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                const SizedBox(height: 4),
                ...channelAnalysis.map(
                  (Map<String, dynamic> channel) => Text(
                    'Ch${channel['channel']}: ${(channel['power'] as double).toStringAsFixed(1)}W | '
          ...
          Container(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                Text(
                  'Available to Share: ${totalSymmSurplus.toStringAsFixed(1)}W',
                ),
                Text('Channels Needing Power: $channelsNeedingPower'),
                Text(
                  'Share Per Needing: ${sharePerNeeding.toStringAsFixed(1)}W',
          ...
          Container(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                for (final Map<String, dynamic> delivery
                    in powerDelivered) ...<Widget>[
                  Text(
                    'Ch${delivery['channel']}: ${(delivery['powerDelivered'] as double).toStringAsFixed(1)}W delivered '
          ...
          Container(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                Text(
                  'Total Power: ${totalPower.toStringAsFixed(1)}W ≤ ${totalCapacity.toInt()}W ${totalPower <= totalCapacity ? "✅" : "❌"}',
                ),
                Text(
                  'Max Circuit: ${circuitData.isNotEmpty ? (circuitData.first['power'] as double).toStringAsFixed(1) : "0"}W ≤ ${asymmetricalShareable.toInt()}W ${circuitData.isNotEmpty && (circuitData.first['power'] as double) <= asymmetricalShareable ? "✅" : "❌"}',
                ),
                Text(
                  'Net Power Sharing: ${netPowerSharing.toStringAsFixed(1)}W ≥ 0 ${netPowerSharing >= 0 ? "✅" : "❌"}',
                ),
                Text(
                  'Result: ${canShare ? "✅ SUCCESS - $ampName can handle this configuration" : "❌ FAILED - $ampName cannot handle this configuration"}',
                  style: TextStyle(
                    fontWeight: FontWeight.bold,
                    color:
                        canShare
      ...
      children: <Widget>[
        ...
        Container(
          ...
          child: const Column(
            ...
            children: <Widget>[
              Text(
                'Power Distribution Formulas:',
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16),
              ),
              SizedBox(height: 8),
              Text(
                'Share_Per_Needing = Available_To_Share / N_needing',
                style: TextStyle(fontFamily: 'monospace'),
              ),
              Text(
                'Power_Available_i = L + Share_Per_Needing (for needing channels)',
                style: TextStyle(fontFamily: 'monospace'),
              ),
              Text(
                'Limit_Power_Delivered_i = Min(Power_Available_i, Asymmetrical_Shareable)',
                style: TextStyle(fontFamily: 'monospace'),
              ),
              Text(
                'Power_Delivered_i = Min(Limit_Power_Delivered_i, Actual_Power_Needed_i)',
                style: TextStyle(fontFamily: 'monospace'),
              ),
            ],
          ),
  ...
  Widget _buildPowerDistributionAnalysis() {
    ...
    if (circuitData.isEmpty) {
      return Container(
        ...
        child: const Text(
          'Select speaker models to see power distribution analysis',
          style: TextStyle(fontStyle: FontStyle.italic),
        ),
      );
    }
    ...
    return Column(
      children: <Widget>[
        ...
        _buildPowerDistributionForAmplifier(
          AmplifierCatalog.psx2404d.name,
          AmplifierCatalog.psx2404d.symmetrical.watts.toDouble(),
          AmplifierCatalog.psx2404d.asymmetrical.watts.toDouble(),
          circuitData,
  ...
  Widget _buildPowerDistributionForAmplifier(
    String ampName,
    double perChannelLimit,
    double asymmetricalShareable,
    List<Map<String, dynamic>> circuitData,
          ...
          Container(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                ...channelAnalysis.map(
                  (Map<String, dynamic> channel) => Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: <Widget>[
                      Text(
                        'Ch${channel['channel']}: ${(channel['power'] as double).toStringAsFixed(1)}W needed',
                      ),
                      if (channel['needsSharing'] as bool) ...<Widget>[
                        Text(
                          '  Power Available: ${perChannelLimit.toInt()}W + ${sharePerNeeding.toStringAsFixed(1)}W = ${(perChannelLimit + sharePerNeeding).toStringAsFixed(1)}W',
                        ),
                        Text(
                          '  Limit Power Delivered: Min(${(perChannelLimit + sharePerNeeding).toStringAsFixed(1)}W, ${asymmetricalShareable.toInt()}W) = ${math.min(perChannelLimit + sharePerNeeding, asymmetricalShareable).toStringAsFixed(1)}W',
                        ),
                        Text(
                          '  Power Delivered: Min(${math.min(perChannelLimit + sharePerNeeding, asymmetricalShareable).toStringAsFixed(1)}W, ${(channel['power'] as double).toStringAsFixed(1)}W) = ${math.min(math.min(perChannelLimit + sharePerNeeding, asymmetricalShareable), channel['power'] as double).toStringAsFixed(1)}W',
                        ),
                      ] else ...<Widget>[
                        Text(
                          '  Power Available: ${perChannelLimit.toInt()}W (no sharing needed)',
                        ),
                        Text(
                          '  Power Delivered: Min(${perChannelLimit.toInt()}W, ${(channel['power'] as double).toStringAsFixed(1)}W) = ${math.min(perChannelLimit, channel['power'] as double).toStringAsFixed(1)}W',
                        ),
                      ],
                      const SizedBox(height: 4),
                    ],
      ...
      children: <Widget>[
        ...
        Container(
          ...
          child: const Column(
            ...
            children: <Widget>[
              Text(
                'Headroom Calculation Formulas:',
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16),
              ),
              SizedBox(height: 8),
              Text(
                'Channel_Headroom_i = 10 × log₁₀(Asymmetrical_Shareable / Power_Delivered_i)',
                style: TextStyle(fontFamily: 'monospace'),
              ),
              Text(
                'Total_Amp_Headroom = 10 × log₁₀(Total_Capacity / Total_Delivered)',
                style: TextStyle(fontFamily: 'monospace'),
              ),
              Text(
                'Effective_Headroom_i = Min(Channel_Headroom_i, Total_Amp_Headroom)',
                style: TextStyle(fontFamily: 'monospace'),
              ),
              Text(
                'Loudspeaker_Headroom_i = 10 × log₁₀(Power_Needed_Without_Offset / Power_Delivered_i)',
                style: TextStyle(fontFamily: 'monospace'),
              ),
            ],
          ),
  ...
  Widget _buildHeadroomAnalysis() {
    ...
    if (circuitData.isEmpty) {
      return Container(
        ...
        child: const Text(
          'Select speaker models to see headroom analysis',
          style: TextStyle(fontStyle: FontStyle.italic),
        ),
      );
    }
  ...
  Widget _buildHeadroomForAmplifier(
    String ampName,
    double perChannelLimit,
    double totalCapacity,
    double asymmetricalShareable,
    ...
    double totalDelivered = 0.0;
    final List<Map<String, dynamic>> headroomAnalysis =
        <Map<String, dynamic>>[];
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (channelPower > 0) {
        final double channelHeadroom =
            10 * math.log(asymmetricalShareable / powerDelivered) / math.ln10;
          ...
          Container(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                const SizedBox(height: 4),
                ...headroomAnalysis.map(
                  (Map<String, dynamic> channel) => Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: <Widget>[
                      Text(
                        'Ch${channel['channel']}: ${(channel['powerDelivered'] as double).toStringAsFixed(1)}W delivered',
                      ),
                      Text(
                        '  Channel Headroom: ${(channel['channelHeadroom'] as double).toStringAsFixed(1)}dB',
                      ),
                      Text(
                        '  Effective Headroom: Min(${(channel['channelHeadroom'] as double).toStringAsFixed(1)}dB, ${totalAmpHeadroom.toStringAsFixed(1)}dB) = ${math.min(channel['channelHeadroom'] as double, totalAmpHeadroom).toStringAsFixed(1)}dB',
                      ),
                      const SizedBox(height: 4),
                    ],
                  ),
  ...
  Widget _buildFinalValidationStep(
    AmpMatchingResult result,
    PowerAllocationStrategy strategy,
  ) {
    return Column(
      ...
      children: <Widget>[
        ...
        Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              Text(
                'Power Efficiency: ${(result.powerEfficiency * 100).toStringAsFixed(1)}%',
              ),
              Text(
                'Channel Efficiency: ${(result.channelEfficiency * 100).toStringAsFixed(1)}%',
              ),
              ...
              if (result.hasErrors) ...<Widget>[
                ...
                const SizedBox(height: 4),
                const Text(
                  '⚠️ Configuration has warnings - see details above',
                  style: TextStyle(
                    color: Colors.orange,
                    fontWeight: FontWeight.bold,
                  ),
        ...
        ...result.assignments.asMap().entries.map((
          MapEntry<int, AmpAssignment> entry,
        ) {
          final int index = entry.key;
          ...
          return Container(
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                Text(
                  '   • Power per Channel: ${assignment.ampModel.symmetrical.watts.toInt()}W',
                ),
                Text(
                  '   • Total Capacity: ${correctCapacity.toInt()}W (${strategy == PowerAllocationStrategy.symmetrical ? "Symmetrical" : "Asymmetrical"} mode)',
                ),
                Text(
                  '   • Utilization: ${assignment.usedChannels}/${assignment.totalChannels} channels (${(assignment.channelUtilization * 100).toStringAsFixed(1)}%)',
                ),
                const Text('   • Assigned Circuits:'),
                ...assignment.circuits.map(
                  (Circuit circuit) => Text(
                    '     - Circuit ${circuit.circuitId}: ${circuit.speakerCount}x ${circuit.model} (${circuit.mode})',
                  ),
                ),
              ],
            ),
  ...
  double _estimateCircuitPower(CircuitInput circuit) {
    ...
    final int speakerCount =
        int.tryParse(circuit.speakerCountController.text) ?? 0;
    final double tapWatts =
        double.tryParse(circuit.tapWattsController.text) ?? 0.0;
    final double offsetDb =
        double.tryParse(circuit.offsetDbController.text) ?? 0.0;
  ...
  Widget _buildTapSelector(CircuitInput circuit) {
    if (circuit.selectedModel == null) {
      return FusionTextFormField(
        semanticId: '${circuit.circuitId}-tap-selector',
        controller: circuit.tapWattsController,
        title: 'Tap Watts',
    ...
    return FusionDropdownButtonFormField(
      semanticKey: "${circuit.circuitId}-tap-selector",
      value: circuit.selectedTap.toString(),
      hintText: 'Tap Watts',
      options: speaker.hiZTaps.map((double tap) => tap.toString()).toList(),
      displayString:
          (String value) => '${double.tryParse(value)?.toStringAsFixed(1)}W',
      // items:
      //     speaker.hiZTaps.map((double tap) {
      ...
      onChanged: (String? value) {
        setState(() {
          circuit.selectedTap = double.tryParse(value ?? '');
          if (value != null) {
  ...
  Widget _buildAsymmetricalFinalValidationStep() {
    ...
    final int totalChannelsUsed = optimizedAllocation.fold(
      0,
      (int sum, Map<String, dynamic> amp) => sum + (amp['usedChannels'] as int),
    );
    final int totalChannelsAvailable = optimizedAllocation.fold(
      0,
      (int sum, Map<String, dynamic> amp) =>
          sum + (amp['totalChannels'] as int),
    );
    final double channelEfficiency =
        totalChannelsAvailable > 0
            ? totalChannelsUsed / totalChannelsAvailable
            : 0.0;

    // Calculate power efficiency (total delivered vs total capacity)
    final double totalSystemCapacity = optimizedAllocation.fold(
      0.0,
      (double sum, Map<String, dynamic> amp) =>
          sum +
          ((amp['tierModel'] as AmplifierModel).asymmetrical.totalCapacity
              .toDouble()),
    );
    final double powerEfficiency =
        totalSystemCapacity > 0 ? totalSystemPower / totalSystemCapacity : 0.0;

    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: <Widget>[
        const Text(
          '✅ Final Mixed Amplifier Allocation Results:',
          style: TextStyle(fontWeight: FontWeight.bold),
        ),
        const SizedBox(height: 8),
        ...
        Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              ...
              Text(
                'Total System Capacity: ${totalSystemCapacity.toStringAsFixed(1)}W',
              ),
              Text(
                'Power Efficiency: ${(powerEfficiency * 100).toStringAsFixed(1)}%',
              ),
              Text(
                'Channel Efficiency: ${(channelEfficiency * 100).toStringAsFixed(1)}%',
              ),
              const SizedBox(height: 8),
              const Text(
                '✅ Mixed amplifier allocation with circuit movement optimization applied',
                style: TextStyle(
                  color: Colors.green,
                  fontWeight: FontWeight.bold,
                ),
        ...
        const Text(
          '🔌 Mixed Amplifier Assignments:',
          style: TextStyle(fontWeight: FontWeight.bold),
        ),
        const SizedBox(height: 8),

        ...optimizedAllocation.asMap().entries.map((
          MapEntry<int, Map<String, dynamic>> entry,
        ) {
          final int index = entry.key;
          final Map<String, dynamic> amplifier = entry.value;
          final AmplifierModel ampModel =
              amplifier['tierModel'] as AmplifierModel;
          final List<Map<String, dynamic>> assignedCircuits =
              amplifier['circuitsAssigned'] as List<Map<String, dynamic>>;
          ...
          return Container(
            ...
            child: Column(
              ...
              children: <Widget>[
                Text(
                  '📦 Amplifier ${index + 1}: ${ampModel.name}',
                  style: const TextStyle(fontWeight: FontWeight.w600),
                ),
                const SizedBox(height: 4),
                Text(
                  '   • Channels: ${amplifier['usedChannels']}/${amplifier['totalChannels']} used',
                ),
                Text(
                  '   • Symmetrical Per-Channel: ${ampModel.symmetrical.watts}W',
                ),
                Text(
                  '   • Asymmetrical Per-Channel: ${ampModel.asymmetrical.watts}W',
                ),
                Text(
                  '   • Total Capacity: ${ampModel.asymmetrical.totalCapacity}W',
                ),
                Text(
                  '   • Channel Utilization: ${((amplifier['usedChannels'] as int) / (amplifier['totalChannels'] as int) * 100).toStringAsFixed(1)}%',
                ),
                const SizedBox(height: 4),
                const Text(
                  '   • Assigned Circuits:',
                ...
                ...assignedCircuits.map(
                  (Map<String, dynamic> circuit) => Text(
                    '     - ${circuit['model']}: ${circuit['speakerCount']} speakers (${circuit['power'].toStringAsFixed(1)}W)',
                  ),
                ),
              ],
            ),
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            ...AmplifierCatalog.allModels.map(
              (AmplifierModel amplifier) => Container(
                padding: const EdgeInsets.all(12),
                decoration: BoxDecoration(
            ...
            Container(
              ...
              child: const Column(
                ...
                children: <Widget>[
                  ...
                  SizedBox(height: 8),
                  Text(
                    '• Symmetrical Per-Channel: Fixed power per channel (cannot be shared)',
                  ),
                  Text(
                    '• Asymmetrical Per-Channel: Maximum power a single channel can withdraw (can be shared)',
                  ),
                  Text(
                    '• Total Capacity: Maximum total power the amplifier can deliver',
                  ),
                  SizedBox(height: 8),
                ],
              ),

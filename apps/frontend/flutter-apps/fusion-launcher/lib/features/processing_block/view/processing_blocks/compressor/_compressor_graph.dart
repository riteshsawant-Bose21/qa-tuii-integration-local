...
class _CompressorGraph extends StatelessWidget {
  const _CompressorGraph({super.key});

  @override
  Widget build(BuildContext context) {
    return Consumer<CompressorController>(
      builder: (BuildContext context, CompressorController controller, Widget? child) {
...
class _CompressorGraphPainter extends StatefulWidget {
  const _CompressorGraphPainter({
    super.key,
  ...
  @override
  State<_CompressorGraphPainter> createState() => _CompressorGraphPainterState();
}

class _CompressorGraphPainterState extends State<_CompressorGraphPainter> {
    ...
    return GestureDetector(
      ...
      onPanUpdate: (DragUpdateDetails details) {
        if (draggedPointIndex != null) {
          ...
          final double dbX = (normalizedX * 60.0) - 60.0; // Convert to -60 to 0 dB range
          final double dbY = (normalizedY * 60.0) - 60.0; // Convert to -60 to 0 dB range
          ...
          if (lastPanPosition != null) {
            ...
            if (deltaX > deltaY) {
              // Horizontal drag
              widget.onPointChanged(draggedPointIndex!, dbX.clamp(-60.0, 0.0), DragDirection.horizontal);
            } else {
              // Vertical drag
              widget.onPointChanged(draggedPointIndex!, dbY.clamp(-60.0, 0.0), DragDirection.vertical);
            }
          }
  ...
  Offset _dbToPixel(num dbX, num dbY, Size size) {
    // Convert dB values (-60 to 0) to pixel coordinates
    final double normalizedX = ((dbX) + 60) / 60; // 0 to 1
    final double normalizedY = 1 - ((dbY) + 60) / 60; // 0 to 1, inverted
...
class _GateCurvePainter extends CustomPainter {
  ...
  void _drawGrid(Canvas canvas, Size size) {
    ...
    for (int db = -60; db <= 0; db += 20) {
      final double x = ((db + 60) / 60) * size.width;
      canvas.drawLine(Offset(x, 0), Offset(x, size.height), gridPaint);
    }

    // Draw horizontal grid lines (every 10 dB)
    for (int db = -60; db <= 0; db += 20) {
      final double y = (1 - (db + 60) / 60) * size.height;
  ...
  Offset _dbToPixel(num dbX, num dbY, Size size) {
    final double normalizedX = ((dbX) + 60) / 60;
    final double normalizedY = 1 - ((dbY) + 60) / 60;

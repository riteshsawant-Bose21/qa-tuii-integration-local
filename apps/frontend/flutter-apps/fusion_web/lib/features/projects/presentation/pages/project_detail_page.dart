...
class _ProjectDetailPageState extends State<ProjectDetailPage> {
  ...
  Widget build(BuildContext context) {
    ...
    final healthyPct =
        totalDevices == 0 ? 0 : ((p.healthyDevices / totalDevices) * 100).round();
    final warningPct =
        totalDevices == 0 ? 0 : ((p.warningDevices / totalDevices) * 100).round();
    final criticalPct =
        totalDevices == 0 ? 0 : ((p.criticalDevices / totalDevices) * 100).round();
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                Row(
                  children: [
                    _actionButton(Icons.edit_outlined, "Edit"),
                    const SizedBox(width: 12),
                    _actionButton(Icons.delete_outline, "Archive",
                        danger: true),
                  ],
                )
              ],
                  ...
                  child: _card(
                    child: Column(
                      ...
                      children: [
                        ...
                        Row(
                          children: [
                            _healthTile("Healthy", p.healthyDevices,
                                healthyPct, Colors.green),
                            const SizedBox(width: 12),
                            _healthTile("Warning", p.warningDevices,
                                warningPct, Colors.orange),
                            const SizedBox(width: 12),
                            _healthTile("Critical", p.criticalDevices,
                                criticalPct, Colors.red),
                          ],
                        )
                      ],
                    ),
            ...
            Row(
              children: [
                _tabButton("Incidents (${p.incidents})",
                    DetailTab.incidents),
                const SizedBox(width: 8),
            ...
            if (_selectedTab == DetailTab.incidents) ...[
              _incidentCard("Intermittent Network Dropout",
                  "Connectivity", "medium", "open"),
              _incidentCard("Speaker Calibration Drift",
                  "Performance", "low", "resolved"),
              _incidentCard("Power Supply Voltage Low",
  ...
  Widget _healthTile(
      String label, int count, int pct, Color color) => Expanded(
        child: Container(
  ...
  Widget _incidentCard(
      String title, String category, String priority, String status) {
    return Container(

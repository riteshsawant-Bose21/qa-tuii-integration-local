...
class ProjectViewModel extends Cubit<ProjectViewModelState> {
  ...
  Future<void> loadAllLocalProjects() async {
    ...
    try {
     // final ResponseCallback<List<ProjectData>> projectsResponse = await projectManager.loadProjectsFromLocal();
      final ResponseCallback<List<ProjectData>> projectsResponse = ResponseCallback.success(dummyProjects);
  ...
  /// Loads all local projects and emits the appropriate state.
  // Future<void> loadSearchedProjects() async {
  //   print("Loading searched projects...");
  //   emit(SearchProjectsLoaded(
  //     isLoading: true
  //   ));
  //   try {
  //     //final ResponseCallback<List<ProjectData>> projectsResponse = await projectManager.loadProjectsFromLocal();
  //     final ResponseCallback<List<ProjectData>> projectsResponse = ResponseCallback.success(dummyProjects);
  //     if (projectsResponse.success) {
  //       allProjects = projectsResponse.data ?? <ProjectData>[];emit(
  //         SearchProjectsLoaded(
  ...
  //
  //         emit(SearchProjectsLoaded(
  //             errorMessage: "Failed to load projects: ${projectsResponse.message}"
  //         ));
  //
  //       return;
  //     }

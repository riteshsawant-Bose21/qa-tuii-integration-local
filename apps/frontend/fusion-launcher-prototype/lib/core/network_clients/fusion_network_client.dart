...
import 'package:fusion_design_tool_prototype/core/constants/telemetry_data.dart';
import 'package:fusion_design_tool_prototype/core/network_clients/rest_client/dio_client.dart';
...
class FusionNetworkClient {
  ...
  String? get accessToken {
    if (_accessToken != null && _expiry != null) {
      ...
      final String? storedAccessToken = serviceLocator<SharedPreferencesHandler>().getString(SharedPreferenceKeys.accessToken);
      final String? expiryTime = serviceLocator<SharedPreferencesHandler>().getString(SharedPreferenceKeys.expiry);
  ...
  String? get refreshToken {
    if (_refreshToken != null && _expiry != null) {
      ...
      final String? storedRefreshToken = serviceLocator<SharedPreferencesHandler>().getString(SharedPreferenceKeys.refreshToken);
  ...
  String geApiUrl(FusionApiEndpoint api, {String? baseUrlToOverride}) {
    ...
    if (api.type == FusionApiType.droServer) {
      return "http://${serviceLocator<AppSettings>().droServerUrl}${api.path}";
    } else if (api.type == FusionApiType.fusionServer) {
      return "http://${serviceLocator<ProjectManager>().value.virtualIP ?? "192.168.29.217"}:8080${api.path}";
    } else if (api.type == FusionApiType.backendServer) {
      return "http://${serviceLocator<AppSettings>().fusionCloudBackendUrl}/api/v1${api.path}";
    } else {
  ...
  Future<ResponseCallback<List<T>>> getList<T>({
    ...
    try {
      ...
      if (isSuccess) {
        ...
        return ResponseCallback<List<T>>(
          success: true,
      ...
      return ResponseCallback<List<T>>(
        success: false,
        message: "Exception in FusionNetworkClient.getList() - $ex",
      );
    }
  }
  ...
  Future<ResponseCallback<T>> post<T>({
    ...
    try {
      ...
      return ResponseCallback<T>(
        success: false,
        message: "Exception in FusionNetworkClient.post() - $ex",
      );
    }
  }
  ...
  Future<ResponseCallback<T>> connect<T>() async {
    try {
      await serviceLocator<TelemetryData>().initializeTelemetryAddresses();
